...
- **티켓 상단 표에 시작 날짜·기한·선행 세 행을 반드시 적는다**(2026-09-21 — 9월 3주차 회고: "시작 날짜가 없어 언제까지 처리인지 알기 어렵다 · 선행 조건을 넣어 달라"). **시작 날짜**는 발행 날짜와 같은 값, **기한**은 중요도의 마감을 날짜로 환산한 값(아래 Jira 절의 환산 기준), **선행**은 먼저 끝나야 하는 티켓의 파일명과 Jira 키(없으면 "없음"이라고 적는다 — 빈칸은 "없음"인지 "안 적음"인지 구분되지 않는다). 티켓이 아닌 선행(사람 손 콘솔 작업·외부 승인)도 여기에 상태와 함께 적는다. 원본이 마크다운이므로 **Jira 필드와 같은 값을 양쪽에 둔다** — Jira에만 있으면 저장소만 보는 사람·세션은 순서를 알 수 없다.
...
티켓의 **원본은 `docs/tickets/`의 마크다운**이고, Jira(`runtime364.atlassian.net` — 프로젝트 `KAN`)는 팀이 함께 보는 표시면이다. 티켓을 발행하면 **양쪽에 올린다** — 이슈 본문 끝에 `원본: docs/tickets/<파트>/pending/<파일>.md`를 적어 원본을 가리키고, 제목은 `[FE] …`, 라벨은 파트(`fe`·`be`·`infra`·`ai`)로 둔다.

처리를 마치면 **같은 시점에** 마크다운을 `archive/`로 옮기고 Jira 이슈를 완료로 넘긴다. 한쪽만 움직이면 어느 쪽이 진실인지 알 수 없게 된다. 보류할 때는 양쪽 다 그대로 두고 보류 사유를 남긴다(`pending/`의 처리 기록 · Jira 코멘트).

**발행할 때 Jira 필드 세 가지를 반드시 채운다.** 비워 두면 보드가 "무엇이 밀렸고 무엇을 먼저 해야 하는가"에 답하지 못해, 결국 매번 마크다운을 열어 봐야 한다.

- **선행관계** — 다른 티켓이 끝나야 시작할 수 있으면 **이슈 링크로 남긴다**(`is blocked by` / `blocks`). 마크다운의 "선행 작업" 서술은 Jira에서 보이지 않는다. 링크가 있어야 앞 티켓이 완료로 넘어갈 때 뒤 티켓이 풀린 것을 보드에서 알 수 있다. 사람 손이 필요한 콘솔 작업처럼 **티켓이 아닌 선행**은 링크 대신 본문에 적고 상태를 함께 쓴다.
- **시작 날짜** — **발행 날짜를 적는다.** 마크다운 상단 표의 발행 날짜와 같은 값이다. 이게 있어야 얼마나 묵었는지가 보인다.
- **기한 날짜** — **중요도의 마감을 날짜로 환산해 적는다**(아래 표 기준). Highest·High는 발행 당일, Medium은 +3일, Low는 그 주 금요일, Lowest는 비워 둔다. 중요도만 적고 날짜를 비우면 "Medium이 3일"을 매번 머릿속으로 환산해야 하고, 지난 것이 눈에 띄지 않는다.

API로 채울 때 필드는 **시작 날짜 `customfield_10015` · 기한 `duedate` · 선행 `issuelinks`**다(전부 `YYYY-MM-DD`).

### 중요도

**착수 순서가 아니라 마감을 정한다.** "중요해 보임"이 아니라 언제까지 끝나야 하는가로 고른다.

| 중요도 | 의미 |
|---|---|
| Highest | **즉시 처리** — 다른 일을 멈추고 잡는다 |
| High | **오늘 안에 처리** |
| Medium | **3일 안에 처리** |
| Low | **이번 주 안에 처리** |
| Lowest | **가장 나중에 처리** — 순서가 밀려도 괜찮다 |

마감이 지켜지지 못할 상황이면 **중요도를 내리는 것이 아니라 그 사유를 티켓에 적는다.** 등급을 조정해 맞추면 목록이 실제 상태를 반영하지 않게 된다.
