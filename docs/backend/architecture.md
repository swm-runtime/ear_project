...
**AI 파이프라인 자체는 AI 서버의 책임으로 분리한다.** Backend는 파이프라인을 *실행*하지 않고 *오케스트레이션*한다. 즉 "어떤 원문을 언제 어떤 순서로 처리할지, 그 결과를 어떤 상태로 저장하고 언제 발행할지"는 Backend가 관리하고, 모델 추론·프롬프트·QA 판정 알고리즘·음성 합성·**임베딩 생성**은 AI 서버 내부에 둔다.

- **임베딩도 모델 추론이므로 같은 경계다**(개정 2026-09-01 — 추천 고도화). AI 서버(`ai-server/`)가 `POST /embeddings`로 생성하고, 벡터 **저장**(`content_embeddings` — pgvector)과 유사도 스코어링은 Backend에 남는다(`domain.md` 5.6). 현재 호출자는 메타데이터 부여 파이프라인 스킬이며(`docs/ai/metadata-pipeline.md` 4.3), Backend가 부르게 되는 것은 재발행·소급·모델 교체의 일괄 재생성 시점이다.
...
- **재생 URL은 저장 경로를 직접 서명한다**(개정 2026-08-31). 형태는 `https://<cdn>/audio/<무작위 hex 32>.<ext>?<서명>`이다. 종전 설계였던 `/play/<contentId>` 재작성(CloudFront KeyValueStore + viewer-request Function)은 **운영 AWS 계정의 조직 SCP가 KVS 데이터 플레인을 명시적으로 거부**해 성립하지 않는다(정책 `p-5soyo0ar`, 실측 2026-08-31 — `docs/infra/architecture.md` 3.2). 매핑 계층이 없어졌을 뿐 보호 수준은 같다 — 키가 무작위라 열거·추측이 불가능하고 서명은 여전히 단기 만료다.
- **회수 시 동작도 그대로다** — 신규 발급이 즉시 차단되고 이미 나간 URL은 만료로 자연 소멸한다. KVS 키 삭제는 추가 방어였을 뿐이라 사라져도 `partner-control.md` 4.3의 처리 순서와 어긋나지 않는다.
- **썸네일은 서명하지 않는다.** 오디오와 같은 비공개 버킷에 두되 `thumb/*` 경로만 CloudFront에서 무서명으로 공개한다. 목록·탐색 화면에 그대로 노출되는 값이라 단기 만료 서명 URL과 수명이 맞지 않는다.
...
- 클러스터에서는 워커별 인메모리라 **실효 한도 = 한도 × 워커 수**다(작은 한도는 나누지 않음 — `infra/scaling.md` 5.2 C, 2026-09-26).
