...
| 선택지 | 내용 |
|---|---|
| **(a)** | `User.tier` 제거. 티어는 항상 `Subscription`에서 유도(행 없으면 free) | 정합성 최선, 조회마다 join |
| **(b) 권고** | `Subscription`이 원천, `User.tier`는 **비정규화 캐시**로 유지. 갱신은 구독 상태 변경 시에만 |
| **(c)** | 현행 유지(양쪽 독립) | 어긋남이 반드시 발생 |
...
| **(b)** | 전부 한 테이블에 넣고 스코어링에서 필터 | 테이블이 빠르게 비대해짐(재생 1회당 수십 건) |
...
> **권고: `ContentStat { content_id, period_type: enum(day|week|month|all), period_date, ... }` PK `(content_id, period_type, period_date)`.**
> `PartnerReport`는 별도 테이블로 만들지 말고 `ContentStat` 집계 + `Content.partner_id` 필터로 산출한다. 단 `source_link_click_count`(원문 유입, FR-34 핵심 지표)를 `ContentStat`에 추가해야 한다.
...
| 선택지 | 내용 |
|---|---|
| **(a)** | 발행·회수 시점에 즉시 갱신 | 정확하지만 파이프라인·회수 로직에 결합 |
| **(b) 권고** | 컬럼을 두지 않고 조회 시 집계 + 캐시 | 주제 수가 수십 개 수준이므로 비용 문제 없음 |
...
| C-6 | `Episode` ↔ `Content` | 1:1인지 확인. 분할 시 Episode N개 → Content N개이므로 1:1로 보임 |
...
- A-1 재생 위치 모델 → 권고 (b)
- A-2 `daily_play_count` 컬럼 제거 + `play_date` 추가 → 권고
- A-3 `tier` 원천 → 권고 (b)
- A-4 `LibraryItem.deleted_reason` 추가 → 권고
- A-5 중복 방지 유니크를 `LibraryItem` 한 곳으로 → 권고
- A-6 `Content.status` 분리 + `Content` 생성 시점 → 권고
- A-7 `UserSignal.action` 범위 축소 + `manual_complete` 추가 → 권고

**답해주면 좋은 것 (B급 8건)** — 전부 권고안 있음. 이견 없으면 권고대로 진행 가능.

**PRD 4건** — 1·2·4번은 컬럼 존재 여부가 걸려 있어 결정 전에는 해당 부분을 비워둬야 한다.
