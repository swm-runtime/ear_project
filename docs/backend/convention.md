...
- **`dev → main` PR에는 `backend/package.json`의 `version`을 올리는 커밋을 포함한다.** 운영 배포가 끝나면 그 값으로 `v<version>` 태그가 붙는다. 버전을 올리지 않으면 `v<version>-<날짜시각>` 형식의 대체 태그가 붙고 경고가 나서, 어느 배포가 어느 버전인지 추적할 수 없게 된다. 규칙: 사용자 영향 있는 기능·정책 변경은 minor(1.1.0), 수정만이면 patch(1.0.1), 계약이 깨지는 변경은 major.
