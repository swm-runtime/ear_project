...
- `main`은 **`dev → main` PR로만** 갱신한다. 배포 기준선이다 — **`main` 머지 = 운영 자동 배포**, `dev` 머지 = 개발계 배포(2026-09-16 전환).
- **버전의 기준은 앱이다**(2026-09-17 개정 — 종전 "main 머지마다 백엔드 버전을 올린다"는 폐기). 제품 버전은 `frontend/app.json`의 `expo.version`(스토어 버전) 하나이고, **`backend/package.json`의 `version`은 앱과 항상 같게 두며 백엔드 혼자 올리지 않는다.**
  - **태그**: 운영 배포가 끝나면 `v<앱 버전>`이 붙는다. 같은 앱 버전에서 백엔드만 다시 배포하면 `v1.0.0+2`, `v1.0.0+3`처럼 **배포 순번만 는다**(`+N`은 SemVer 빌드 메타데이터 — "같은 버전의 다른 빌드"). 어떤 이미지가 나갔는지는 태그 메시지의 ECR 이미지 URI로 추적한다.
  - **앱이 스토어 버전을 올릴 때**는 그 PR에서 `backend/package.json`도 같은 값으로 올린다 — `cd backend && npm version <버전> --no-git-tag-version`(`package-lock.json`까지 함께 바뀐다). 다르면 PR의 필수 체크 **"원본 브랜치 확인 (dev)"** 가 막는다. **dev로의 PR에서도 검사한다** — main PR에서만 보면 어긋남이 dev에 들어간 뒤 릴리스 때에야 드러나 PR을 한 번 더 열어야 한다. 이 수정은 파트 경계(자기 파트만 수정)의 **유일한 예외**다.
  - **태그가 뜻하는 것**: "그 앱 버전 시기의 **운영 API 배포**"다. 스토어 출시 표시가 아니다 — 심사·승인과 무관하게 main 배포 시점에 붙고, 백엔드 변경이 없는 main 머지(프론트만)에는 붙지 않는다. 운영 배포가 실패하면 태그도 없다.
  - 번호는 기존 `+N` 중 **최댓값 + 1**이다(개수가 아니다 — 중간 태그가 지워져도 겹치지 않는다). 같은 커밋을 재실행하면 태그를 새로 붙이지 않는다.
  - **왜 바꿨나**: 백엔드는 자주, 앱은 스토어 심사 때문에 드물게 나간다. 백엔드가 번호를 정하면 곧 `v1.0.5` 태그가 **존재하지 않는 앱 버전**을 가리켜, "앱 1.0.5가 나갔다"로 읽힌다.
  - 올리는 기준은 앱 릴리스에 따른다: 사용자 영향 있는 기능·정책 변경은 minor(1.1.0), 수정만이면 patch(1.0.1), 계약이 깨지는 변경은 major.
  - OTA 업데이트(`eas-update`)는 스토어 버전을 바꾸지 않으므로 태그와 무관하다. 서버 env의 `LATEST_APP_VERSION_IOS/ANDROID`도 **이것과 합치지 않는다** — 스토어 심사가 플랫폼마다 따로 끝나 iOS만 새 버전인 기간이 생긴다.
- `dev → main` PR은 작성자 외 **1명 승인**이 필수다(브랜치 보호). 승인은 댓글이 아니라 Review → Approve로 제출한다.
