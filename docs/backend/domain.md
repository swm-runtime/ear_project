...
- 인덱스 정의의 **`DESC` 표기는 조회 의도**(최신순 읽기)를 적은 것이다. 실제 인덱스는 TypeORM `@Index`가 만드는 ASC이며, btree는 양방향 스캔이라 **방향이 한 가지인 복합 인덱스에서는 결과·비용이 같다.** 방향이 섞인 복합 인덱스가 필요해지는 시점에만 코드에 방향을 명시한다(감사 2026-09-09 하 #10 — 표기 통일).
...
topics
  ...
  is_visible                boolean         DEFAULT false  ★켜기는 관리자만 (합의 2026-08-06) · 0건 자동 숨김 (2026-09-17)
  display_order             int

idx_topics_is_visible_display_order (is_visible, display_order)
```

- **`content_count` 컬럼을 두지 않는다** (B-7). 관리자 화면에서 필요할 때 `content_topics` COUNT로 집계한다(삭제 판정은 원시 연결 건수, 노출 판정은 `contents`와 조인한 노출 가능 건수 — `admin.md` 4.5). 주제 수가 수십 개 수준이라 비용 문제가 없다.
- **`is_visible`을 켜는 것은 관리자만 한다.** 시스템이 바꾸는 경우는 **노출 가능 콘텐츠(`published` + 라이선스 미만료)가 0건이 된 노출 주제를 `false`로 내리는 것 하나뿐이다**(개정 2026-09-17, KAN-58 — `admin.md` 4.5). 내리기 한 방향·한 규칙·한 서비스(`TopicExposureService`)로 제한해 갱신 주체를 고정한 목적(어긋남 방지)을 지킨다. 0건 주제를 켜는 요청은 서버가 거부한다.
...
- **노출 통제(`is_visible`)는 신규 주제의 공개 전 단계에만 쓴다**(합의 2026-08-06). 이미 선택한 사용자가 있는 주제를 숨기는 운영은 하지 않으며, 품질 문제 콘텐츠는 숨김이 아니라 **회수**로 처리한다(`admin.md` 4.5). **예외는 0건 자동 숨김이다**(2026-09-17) — 선택한 사용자가 있어도 내리지만, 그 주제에는 이미 노출 가능 콘텐츠가 없다.
...
- `is_counted`는 **재청취 창의 기산점이 되는 행**인가다 — 차감이 발생한 재생, 그리고 **한도 없는 티어(`plans.daily_play_limit = null`)의 재생**(차감은 없지만 구독으로 지불된 재생 — 결정 2026-08-11, `paywall.md` 4.3-1)이 `true`이고, **재청취 창 안의 재생**(차감 없음)으로 만들어진 행만 `false`다. 창 안의 재청취도 `listened_sec` 적산을 위해 행이 필요하므로, 행의 존재만으로는 기산점을 셀 수 없어 플래그로 가른다.
- 알려진 파급 하나 — 같은 서비스 날짜 안에 한도 없는 티어에서 한도 티어로 강등되면, 강등 전의 재생(`true`)이 그날의 `daily_play_count`에 포함된다. 그날 이미 들은 만큼이 한도에서 차감된 것으로 보이는 동작인데, 재청취 창이 그 콘텐츠들을 다시 열어 주므로 실사용 피해는 없다.
