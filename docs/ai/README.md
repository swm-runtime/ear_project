...
이 디렉토리는 **ear 콘텐츠 파이프라인** — 공개 소스에 근거해 고정 진행자 2인(윤아·이음)의 대화형 팟캐스트(편당 15분)를 제작하는 절차 — 의 명세·프롬프트 자산·운영 DB 스키마를 담는다. **기준 문서는 [PIPELINE.md](PIPELINE.md)** 다 (2026-08 개편 — 구 `pipeline.md`를 대체).
...
| [spec/09-quality-cycle.md](spec/09-quality-cycle.md) | 품질 개선 사이클 — 예시·규칙·피드백의 승격 규약, 프롬프트 버전 관리, 비평가(선행 검수기) |
...
| [skills/critic/rubric.md](skills/critic/rubric.md) | 비평 루브릭 (`critic-vN`) — spec/09 7장 비평가의 검사 항목·리포트 규격 |
| [skills/qa/prompt.md](skills/qa/prompt.md) | QA 실행 프롬프트 (`qa-vN`) — spec/05의 이식본 |
| **기타** | |
| [supabase/schema.sql](supabase/schema.sql) | 운영 DB 스키마 원본 (`topics`·`domains`·`sources`·`backlog`·`runs`) — 변경은 이 파일 기준 |
...
| 도메인 편입·판정 (사람) | spec/01 4장 체크리스트 → Supabase 테이블 에디터 |
| 스윕·군집화 실행 | spec/02 · spec/03 |
| 후보 승인 (게이트 1) | spec/03 6장 |
| 대본 생성 | spec/04(규격) + skills/draft/guidelines.md(현행 프롬프트) + examples/(골드) |
| QA 실행 | skills/qa/prompt.md — **새 컨텍스트에서** (spec/05 5장 독립 실행 규약) |
...
| DB 컬럼·enum 확인 | supabase/schema.sql |
...
작업 폴더에는 있으나 레포에 넣지 않는다 (상태의 원본은 운영 DB, 산출물은 S3 이관 예정 — spec/08 2장):

- `episodes/` — 제작 산출물 (대본·claims·발췌·QA/비평 리포트·피드백). **발췌(`sources.md`)는 재배포 금지 증적**이다
- `sources/` · `backlog/` — 스윕 원본·로컬 백로그 (DB 이관 완료분)
- `references/` — 롤모델 쇼 전사본(저작권)과 구조 분석 노트
- `archive/` — 2026-08 개편 이전의 제안·검토 문서
