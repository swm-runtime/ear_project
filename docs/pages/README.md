...
| [auth.md](auth.md) | 소셜 로그인(카카오·네이버·구글), 로그아웃, 회원 탈퇴 | FR-01, FR-02 |
...
| 12 | **온보딩 3단계 표본 부족 대응** | 인기 콘텐츠를 판별할 재생 데이터가 부족하면 **랜덤 3개를 같은 자리에 배치**한다. 후보 필터는 그대로 적용하고 **사용자+세션 시드로 고정**해 뒤로 가도 섞이지 않는다. 섹션 제목도 "이번 달 인기"에서 바뀐다 | onboarding 4 |
...
| 14 | **카피에 쓰는 수치의 근거** | 알림 사전 안내의 하드코딩된 "20%"를 `N%`로 교체. **개발자가 출처 있는 통계를 찾아 입력**하고 근거를 카피 리소스와 명세 양쪽에 기록한다. 못 찾으면 **정성 표현으로 폴백**하고, 출시 후 자체 측정값으로 대체한다 | onboarding 4, notification 4.1 |

### 🟡 아직 남은 것

| # | 항목 | 내용 | 관련 문서 |
|---|---|---|---|
| 15 | **유료 티어 값** | 데일리·프로의 재생 한도·드립 편수·가격. 시범 운영 후 결정 — `plans` 행의 값만 채우면 되고 마이그레이션 불필요 | subscription, paywall |
| 16 | **P0/P1 최종 확정** | 특히 FR-18(자동 확장), FR-19(푸시), FR-22(검색)의 MVP 포함 여부 | 전체 |
...
| 20 | **알림 사전 안내 재노출 경로 — P0** | #13의 로딩 중에 앱을 강제 종료하면 `onboarding_completed`가 이미 `true`라 **알림 사전 안내를 영영 보지 못한다.** `onboarding_completed`를 나중에 찍으면 편성이 끝내 실패한 사용자가 온보딩에 갇히므로 그 방향은 쓸 수 없다. **(a) 라이브러리 첫 진입 시 1회 노출** / **(b) 설정 알림 항목에 유도 배너 상시 노출** 중 택일 필요. **OS 권한은 거부되면 앱에서 되돌릴 수 없어 미룰수록 손실이 확정된다** | notification 4.1·미결, onboarding 4 |
...
| # | 항목 | 잠정 결정 | 관련 문서 |
|---|---|---|---|
| 22 | **재생 카운트 시점** | 재생 버튼 탭이 아니라 **재생 시작 시점**. 같은 날 같은 콘텐츠 재생은 1회만 카운트(`play_records` 유니크가 보장) | paywall 4.2 |
| 23 | **완청 판정 기준** | 90% 도달. 단 시크로 점프한 경우는 제외(`max_reached_sec` 사용) | player 4.4, library 4.3 |
| 24 | **수동 완료 표시** | **기능 자체를 삭제했다.** 상태는 실제 재생 결과로만 바뀐다 | library 4.3, player 4.4 |
| 25 | **드립 재적립 제외 범위** | 담기 해제·라이브러리 삭제·재생·적립을 **경로 구분 없이 전부 영구 제외** | drip-scheduling 4.2, library 4.4, explore 4.3 |
...
| 32 | **무료 티어 광고 형태** | PRD 4.1의 "광고 제공"이 오디오 프리롤인지 배너인지 미정 → 플레이어 명세에 영향 | paywall, player |

## 개발 착수 순서 제안

`next_doing.md` 1장의 순서를 따릅니다.

1. ~~PRD 내부 충돌 확정~~ → **완료.** 위 표의 14건이 모두 결정됐고 `domain.md`에 반영됐다. 다만 **#20(알림 사전 안내 재노출)은 P0이며 화면 개발 전에 정해야 한다** — OS 권한은 거부되면 되돌릴 수 없다
