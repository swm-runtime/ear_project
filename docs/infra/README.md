...
| [`incident-playbook.md`](incident-playbook.md) | **장애 대응: 증상 색인 · 경보 역색인 · 범주 코드 A~E · 사후 기록** | **뭔가 잘못됐는데 무엇부터 봐야 할지 모를 때** |
| [`runbook.md`](runbook.md) | 운영 절차: 처음부터 재구축 · 코드 배포 · 콘텐츠 관리 · 장애 대응 | 손을 움직여야 할 때 |
| [`scaling.md`](scaling.md) | 확장 계획: 실측 상한 → 로드밸런서를 지금 달지 않는 이유 → 단계별 전환 기준 → 클러스터링 선행 작업 | 트래픽이 늘 것 같을 때, "서버를 키워야 하나" 싶을 때 |
