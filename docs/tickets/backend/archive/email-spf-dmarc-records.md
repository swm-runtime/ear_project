...
| Jira | [KAN-31](https://runtime364.atlassian.net/browse/KAN-31) |
...
- **반영 날짜: 2026-09-20**(DNS 레코드는 2026-09-07 에 반영됐다 — 위 상태 줄).
- 원본과 Jira 를 대조하다 **Jira 만 완료로 넘어가 있는 것**을 찾았다(2026-09-20). PM 이 Jira 에서 직접 닫은 것으로 확인돼, 원본을 같은 상태로 맞춘다.
- 이 시점까지 남아 있던 항목(헤더 확인·2주 관찰·MX 결정)은 PM 판단으로 닫는다. 다시 필요해지면 새 티켓으로 연다.
