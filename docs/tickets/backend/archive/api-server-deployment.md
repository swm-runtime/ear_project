...
## 처리 기록 (반영 날짜: 2026-09-07 — 마지막 두 조건 닫힘)

- **iOS 애플 로그인 nonce 대조** — ✅ TestFlight 빌드 3(1.0.0)에서 애플 로그인 성공을 팀이
  실기기로 확인(2026-09-07). 이로써 `apple-nonce-hash-encoding-mismatch.md`(archive)가
...
(2026-09-06 실서버 대조). 발행 당시의 블로커("프론트가 실기기에서 아무 API도 부를 수 없다")
는 물론, 부수로 만들어진 CI 자동 배포(`deploy-api.yml` — dev 머지 = 실서버 배포)까지 가동
확인된 상태로 닫는다.
