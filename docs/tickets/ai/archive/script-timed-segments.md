...
- Jira KAN-72 → 완료로 전환.
- 2026-09-20 — **소급 구현**(별도 작업으로 두었던 것을 같은 날 구현): `script_align` 작업(0022) — 발행본 dist.mp3 + 대본 → ElevenLabs Forced Alignment → `script-segments.json`. 발행 목록에 자막 셀([자막 뽑기]/[반영])과 "자막 없는 것 전부 뽑기". 대상은 서비스 발행분만(박수헌 결정). 오디오 무변경 → `script_file` 단독 PATCH.
