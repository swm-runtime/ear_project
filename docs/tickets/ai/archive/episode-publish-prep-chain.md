...
## 진행 기록 ② (2026-09-10 — 이미지 모델 `gpt-image-2` 확정)

티켓의 **"열린 것"** 이었던 *"mini 가 참조 이미지(edit) 경로에서 앵커 화풍을 얼마나 따르는지 — 첫 5편 실측 후 모델 확정"* 을 닫는다.

**확정: `gpt-image-2`** (요청자 2026-09-10, 같은 에피소드로 두 모델을 뽑아 비교한 뒤).

### 실측 (`runs` phase `thumbnail`, 앵커 없이 1024×1024 medium)

| 모델 | 소요 | 산출 | 1장 정가 | 월 100편 |
|---|---|---|---|---|
| `gpt-image-1-mini` | 25.6초 | 1,482KB | $0.009 | **$0.90** |
| `gpt-image-2` | 45.3초 | 1,616KB | $0.030 | **$3.00** |

`cost_usd`는 **코드의 정가표 환산값이지 실청구액이 아니다** — 이미지 API는 응답에 비용을 싣지 않는다. 실제 청구는 OpenAI Usage 대시보드에서 확인한다. 특히 `gpt-image-2`는 해상도 과금이라 계산기마다 $0.006~0.053으로 편차가 컸고, 표에 넣은 $0.03은 중간값이다.

### 왜 3배 비싼 쪽을 골랐나

채택 기준이 **"품질보다 콘텐츠마다 일정하게"**(요청자)이기 때문이다. 썸네일은 목록에 여러 장이 나란히 붙어 나오므로, **편마다 화풍이 튀면 한 장 한 장이 좋아도 화면이 무너진다.** 월 3천원 차이는 그 기준 앞에서 판단 근거가 되지 못한다.

20초 차이도 연쇄(TTS → 썸네일 → 패키지)에서는 무시할 수준이다 — TTS가 훨씬 오래 걸린다.

### 반영

- `config.ts` 기본값을 `gpt-image-2`로 바꿨다. **env가 없는 환경에서도 확정값으로 뜨게** 하기 위해서다 — 서버 `env.prod`에만 두면 새 워커·로컬이 mini로 떨어진다
- `env.prod.example` 주석에 확정 사실과 사유를 적었다
- 서버 `env.prod`는 이미 `gpt-image-2`로 반영·재기동 완료

### 이 확정으로 풀린 것

모델이 정해져 **소요 시간·비용이 고정**됐으므로, 미뤄 뒀던 1번(연쇄)·4번(업로드 화면 자동 채우기)의 진행 표시·타임아웃·건너뛰기 규칙을 이제 설계할 수 있다.

**다음은 4번부터.** 버튼을 3번 누르는 것은 귀찮은 정도지만, 썸네일을 내려받아 업로드 화면에 다시 붙이는 것은 **편당 사람이 실수할 수 있는 수작업**이다(현재 `publish/upload`의 `thumbFile`은 발행 필수 조건인데 `<input type="file">`로만 채워진다).

### 아직 열려 있는 것

- **스타일 앵커 미지정** — `THUMBNAIL_ANCHOR_KEY`가 비어 있어 지금은 매 생성이 참조 없이 돈다. 마음에 드는 1장을 골라 지정하면 이후 생성이 그 화풍을 따라간다. **앵커를 붙인 뒤 화풍 일관성을 다시 봐야 한다** — 위 판정은 앵커 없는 상태의 비교다
- **품질 등급은 `medium`** — `high`는 mini·image-2 모두 $0.05대로 비슷해진다. 필요하면 `THUMBNAIL_QUALITY`로 올린다

## 처리 기록 (2026-09-10 — 전 항목 반영, 티켓 종료)
...
요청 1~6과 5-1까지 전부 반영했다. 요청 밖에서 **스타일 앵커를 화면에서 지정하는 기능**이 추가됐다(요청자 2026-09-10).

### 반영 (진행 기록 ①·②에 없는 것)

**1. [발행 준비] 연쇄** — `tts → thumbnail → package`. 남은 단계를 payload `chain` 배열이 들고 다닌다. 단계마다 "다음이 뭔지"를 분기하지 않는 이유는, 분기를 두면 **시작 지점이 다른 세 버튼이 각자 다른 경로를 타고 그중 하나만 고쳐지기** 때문이다. `chain`은 브라우저에서 오는 값이라 화이트리스트(`tts`·`thumbnail`·`package`)로 거른다 — `draft` 같은 AI 작업이 `requires_ai=false`로 주입되는 것을 막는다.

**연쇄로 따라오는 단계에는 `force`를 전달하지 않는다.** 전달하면 [음원 다시 변환]이 썸네일까지 재과금한다.

**3번 건너뛰기 규칙** — TTS에는 그런 규칙이 아예 없었다. 이 티켓에서 정한 대로 **음원이 있고 그 뒤로 대본이 바뀌지 않았으면 건너뛴다.** 판정은 S3 `LastModified`로 한다 — `WORK_ROOT`는 캐시라 로컬 파일 시각을 믿을 수 없다(다른 워커가 만든 산출물은 내려받은 시각이 찍힌다). **건너뛰어도 연쇄는 이어진다** — 여기서 멈추면 [발행 준비]가 패키지에 닿지 못한다.

**3-1. `one_liner`** — 대본 완료 보고 스키마에 필수 필드로 추가했다(추가 호출·비용 0). `episodes.one_liner` → `upload-meta.json` → 발행 설명 첫 줄. **에피소드 대본 탭에서 사람이 고칠 수 있다**(40자 초과 경고). 구 방식(single) 초안은 이 필드를 내지 않으므로 null이고, 썸네일 단계가 설계 축으로 대체한다.

**3-2. 띠 색 설정 화면** — 설정에 패널 신설. HEX 옆에 색 미리보기가 뜬다. 비우면 워커 기본값. **화면에는 `MAJOR_TOPICS` 7개만 띄운다** — 프롬프트 자산의 색 표에는 "배움"이 있는데 분류 체계에는 없다(무해한 잉여값이라 자산은 그대로 뒀다).

**4. 업로드 화면 자동 채우기** — 오디오가 이미 쓰던 방식을 그대로 미러링했다(`has_thumbnail` + `?thumbnail=1` 중계 + 제출 시 File 조립). S3에 브라우저 CORS를 열지 않기 위해 서버가 중계하는 구조도 같다. **파일을 고르면 그쪽이 이긴다** — 교체 경로를 막지 않는다. `upload-meta.json`에 `thumbnail_key`도 실었다.

**5. 진행 표시** — 연쇄가 도는 동안 `TTS ✓ · 썸네일 진행 중 · 패키지 대기` 한 줄. 상단 버튼은 [발행 준비] 하나로 줄이고 개별 재실행은 각 산출물 탭으로 옮겼다. 티켓이 정한 대로 **패키지만 다시 만드는 버튼은 두지 않았고** `package-button.tsx`는 삭제했다.

**5-1. 재발행 — 썸네일 교체** — **발행 이후 다시 만들어진 것만 보낸다.** 안 바뀐 파일까지 올리면 내용이 같은데도 `content_version`이 오르고, 앱은 그것을 새 버전으로 읽어 **저장 위치를 초기화한다.** 버튼 문구가 `오디오 교체`/`썸네일 교체`/`오디오·썸네일 교체`로 갈리고 `publish_log.parts`에도 바뀐 것만 남는다. 제품 API(`PATCH /admin/contents/:id`)는 이미 `thumbnail` 필드를 받으므로 **백엔드 작업은 없었다.**

**6. 문서** — `spec/07` 순서를 `TTS → 썸네일 → 패키지 → 게이트 2`로 고치고 썸네일 규격 절(1-1)을 신설했다. `spec/10`의 큐 종류·연쇄·스키마, RUNBOOK 토글 표(`OPENAI_API_KEY`·`THUMBNAIL_*`)도 갱신했다.

### 요청 밖 추가 — 앵커를 화면에서 지정

에피소드 **썸네일 탭**에 [이 썸네일을 앵커로 지정]을 뒀다. **설정이 env를 이긴다**(`settings.thumbnail.anchor` → 없으면 `THUMBNAIL_ANCHOR_KEY`).

- 그림을 보면서 눌러야 하므로 설정 화면이 아니라 **썸네일 탭**에 뒀다. 설정 화면에서는 현재 앵커가 어느 편인지 확인만 된다
- **에피소드 파일을 가리키지 않고 복사한다** — 그 편을 다시 뽑으면 같은 키를 덮어써 **앵커가 소리 없이 바뀌고** 이후 생성 전부의 화풍이 따라 움직인다
- 티켓이 제안한 `assets/thumbnail/anchor.png`는 쓸 수 없다 — IAM 정책과 코드 `ALLOWED_PREFIX`가 `episodes`·`sweeps`·`datasets` 셋만 허용한다. `datasets/thumbnail/anchor.png`로 넣었다

### 검증

- 웹 **프로덕션 빌드 성공**(실제 배포와 같은 경로) · 워커 타입 검사 오류 0
- 연쇄 판정 로직을 케이스별로 확인 — 세 버튼의 시작 지점, 종료 조건, **오염된 `chain` 값이 걸러지는 것**까지
- 검증 중 이 체크아웃의 `npm install` 누락(`marked`·`linkedom`·`@mozilla/readability`)이 드러나 설치했다. 그전에는 빌드 자체가 안 됐다

### 남은 것

- **앵커를 붙인 뒤의 화풍 일관성은 계속 봐야 한다.** 소재까지 베끼면 `anchor-v2`로 조인다 — 자산이라 배포 없이 즉시 반영된다
- 앵커 **이미지 자체를 올리는** 화면은 없다(기존 썸네일 중에서만 고른다). 필요해지면 별도 요청으로
