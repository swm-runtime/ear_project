...
### 결정이 필요한 것 — 어디서 돌릴 것인가

**안 A — GitHub Actions 러너에서 돌리고, 통과해야 EC2로 배포한다 (제안)**

- e2e용 Postgres는 러너의 **서비스 컨테이너**로 띄운다(`docker-compose.yml`의 로컬 구성과 같은 이미지). env는 `.env.example` 기준 + 테스트용 더미 값이면 충분하다 — 운영 시크릿이 필요 없다.
- 장점: **운영 서버에 부하도 위험도 주지 않는다.** 실패하면 SSH·SG 개폐 자체가 일어나지 않아 배포 경로가 짧아진다.
- 단점: 러너에서 `npm ci` + 빌드 + e2e까지 도는 시간만큼 배포가 늦어진다(체감 수 분).

**안 B — EC2에서 배포 직전에 돌린다**

- 장점: 실제 배포 대상 환경에서 검증된다.
- 단점: **운영 인스턴스에서 테스트 DB를 띄우고 지워야 한다.** 운영 DB를 건드리는 사고 여지가 생기고, 배포 중 자원을 더 쓴다. 실패 시 이미 `git pull`이 끝난 상태라 롤백 절차가 필요하다.

**안 A를 제안한다.** 검증은 배포 대상 환경이 아니라 **배포 전에** 끝나야 하고, 운영 인스턴스에서 테스트 DB를 다루는 위험이 이득보다 크다.
...
- **`CI_SSH_KEY_API` 미설정 시의 조용한 건너뛰기와의 관계.** 현재 guard는 설정 전까지 배포를 건너뛴다. **검증 job은 그 guard와 무관하게 항상 돌아야 한다** — 배포하지 않는 저장소에서도 테스트는 의미가 있다. guard는 배포 job에만 남긴다.
...
- Given e2e 실행 / When 로그를 본다 / Then 운영 DB가 아니라 검증 전용 PostgreSQL에 붙는다
