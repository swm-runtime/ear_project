...
| UptimeRobot | **알림은 Grafana 합성 모니터링으로 옮기고, 계정과 모니터는 알림만 끄고 유지** | 무료 한도 API 테스트 월 10만 회, 5분 헬스체크는 월 8,640회. 다운 알림이 다른 알림과 같은 Slack 채널로 나간다. 지금은 이메일 알림뿐이라 잘 안 보인다. 유지하는 이유는 Grafana Cloud 자체 장애 때 외부 감시가 함께 사라지지 않게 하려는 것 |
