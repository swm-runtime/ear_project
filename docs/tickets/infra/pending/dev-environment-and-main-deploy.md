...
| 상태 | **진행 중** — 1·2·3단계 · 4-1~4-3 · 5-스냅샷 완료(2026-09-15). **대기: 4-4 운영 pull 전환(팀 공유 후 사용자 결정) · 5-알람(미결 — 헬스체크 방식·알림 채널 결정 필요)** · 다음 6단계 |
...
5. 스냅샷 · 알람
   ...
   - 5-알람 **미결(2026-09-15)** — 기존 Slack 알림(ERROR 감시·자원 임계·백업 실패)이 전부 "자기 보고"라 서버가 죽거나 크론이 안 돌면 조용하다. 필요한 건 (b) 외부 헬스체크와 (c) 백업 심장박동 알람 두 개로 좁혀졌고, (a) ERROR·(d) 자원 알람은 중복이라 제외. **결정 필요**: 헬스체크 방식(Route 53 월 0.5달러 / UptimeRobot 무료 / Lambda) · 알림 채널(SNS 메일 그대로 vs AWS Chatbot→Slack). 결정되면 30분 작업
