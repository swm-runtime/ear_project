...
**A안 채택**(사용자 결정 2026-09-09). `pull_request` 트리거의 `paths`를 떼고, verify job 첫
단계에서 변경 파일을 조회해 백엔드 변경 여부를 판정한 뒤 나머지 단계에 그 조건을 걸었다.
관계없는 PR은 단계를 전부 건너뛴 채 **job 결론을 success로 보고한다** — 그래야 필수 체크가
pending에 걸리지 않는다.

`dorny/paths-filter` 대신 `gh api`로 직접 판정했다(배포 파이프라인에 외부 의존을 늘리지 않는다).
`push`·`workflow_dispatch`에서는 판정 자체를 건너뛰고 `backend=true`로 고정한다 — 여기서
false가 나오면 `needs: verify`인 배포 job이 같이 죽는다.

**브랜치 보호 설정 완료.**

```
필수 체크    검증 (lint · build · 유닛 · e2e)   ← job name 과 문자열 일치 대조함
strict       false          머지 전 브랜치 최신화를 요구하지 않는다
enforce_admins  false
required_pull_request_reviews  없음
restrictions    없음
```

**`배포 (EC2)`는 필수 체크에 넣지 않았다.** PR 이벤트에서 항상 `skipped`로 보고되는데 GitHub은
skipped 필수 체크를 통과로 세지 않는다 — 넣었으면 모든 PR이 막혔다.

### 완료 조건 확인

| 조건 | 결과 |
|---|---|
| 검증 실패 PR은 머지가 막힌다 | ✅ 필수 체크 등록 완료 |
| 비백엔드 PR도 결론을 보고한다 | ✅ 실증 — 문서 1건 PR에서 전 단계 skipped·job success(22초) |
| `GET /branches/dev/protection`이 200 | ✅ 체크 이름 일치까지 대조 |
| 팀 합의 없는 항목 미설정 | ✅ 리뷰 필수·관리자 강제·제한 전부 없음 |

**배포 경로 회귀 확인**: #258 머지 직후 dev push 런에서 `검증` → `배포 (EC2)` 둘 다 success.

### `main` 보호 — 걸지 않는다 (판단)

`main`은 `dev → main` PR로만 갱신되고 `dev`가 막히면 깨진 코드는 앞에서 걸러진다. 반면 같은
체크를 `main`에 필수로 걸면 릴리즈 PR이 `backend/**`를 포함할 때마다 방금 dev에서 통과한
커밋을 한 번 더 돌려야 머지된다. `main`은 랜딩 Vercel 프로덕션 추적 대상이라 릴리즈가 막히는
비용이 더 크다. 필요해지면 그때 dev와 같은 방식으로 건다.

### 주의 — 이후 워크플로를 고칠 때

**verify job의 `name`이 곧 필수 상태 체크 이름이다.** 바꾸면 보호의 체크 목록이 끊어져 모든
PR이 pending에 걸린다. 바꿀 때는 보호 설정도 같이 갱신한다.

설정 직후 **이미 열려 있던 PR들은 체크 보고 이력이 없어 pending으로 뜬다.** 커밋을 하나
올리거나 워크플로를 재실행하면 풀린다.
