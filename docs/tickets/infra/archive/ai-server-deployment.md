...
- **반영 날짜: 2026-09-01** — infra 담당의 배포 완료 통보로 archive 이동(사용자 지시).
- **미확인 사항을 남긴다**: 배포 아키텍처·접속 주소·`INTERNAL_AUTH_TOKEN` 발급 내역이 백엔드에 공유되지 않아 **완료 조건의 원격 검증(health 200 · `/embeddings` 단위 벡터 · 무토큰 401 · 토큰 전달)을 수행하지 못했다.** 주소·토큰 수령 시 완료 조건 4건을 재현 확인한다.
- OpenAI API 키는 팀 합의 대기(사용자 통보 2026-09-01) — 그전까지 서버는 stub 또는 키 미주입 상태일 수 있다. "결정 필요" 2건(스킬 접근 경로·API 서버 망 배치)의 결론도 미공유 — infra에 회신 요청.
- **정정 (2026-09-01)**: 위 "배포 완료 통보"와 달리 AI 서버는 **아직 생성되지 않았다**(`docs/infra/inventory.md`에 없음). 구축은 파이프라인 파트가 맡아 `pipeline/deploy/`로 진행한다(`docs/ai/spec/10-webapp.md` 2장 호스트·M6 — 기존 기본 VPC 퍼블릭 서브넷에 EC2 1대, ai-server + pipeline web·io 워커 동거). "결정 필요" 2건의 결론: 스킬 접근 경로 = SSH 터널(공개 도메인 없음) · 망 배치 = 제품 서버와 같은 VPC·서브넷, 8000은 제품 SG에서만. 새 티켓은 만들지 않으며(담당자 본인) 이 문서는 요구사항 기록용으로 archive에 둔다.
