...
## 처리 기록 (반영 날짜: 2026-09-09 — 배선 완료로 전체 완료)

백엔드가 주입 배선을 반영했다(PR #270 — `apply-secrets.py` + `push.sh`, 권고했던 **안 1(배포
경로)** 그대로). 머지 직후 실배포에서 실측 확인:

- 배포 로그 `[secrets] 8 개 항목 갱신` → 컨테이너 재기동 → 헬스 200 (`env.validation.ts` 통과)
  — **완료 조건 2 충족.** 완료 조건 1(선택안·근거 기록)은 위 처리 기록(B안 확정)으로 이미 충족,
  완료 조건 3은 A안 전용이라 해당 없음.
- 값 교체 절차는 `put-secret-value` 후 재배포로 확정(`backend/deploy/aws/README.md` 명시).

남겨 두는 것: "참고" 절의 **노출 이력 값 재발급**은 이 티켓 범위 밖 권고 그대로다 — 이제
절차가 시크릿 갱신 + 재배포로 단순해졌으니 실행 부담은 낮아졌다.
