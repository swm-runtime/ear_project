...
| 상태 | pending — 재생 검증 **충족**(2026-09-07). 이메일 인증 1건만 남았고 **SES 샌드박스 확인이 선행** |
...
> **이것이 진짜 위험이다.** SES 계정이 아직 샌드박스면 **검증된 주소로만 발송된다** —
> 실사용자 이메일은 조용히 실패한다. 코드도 서버 설정도 다 맞는데 메일만 안 간다.
> 발송 테스트보다 **샌드박스 해제 여부 확인이 먼저다.**

### 남은 절차

1. `aws sso login` 후 `aws ses get-send-quota --region ap-northeast-2` —
   `Max24HourSend`가 **200이면 샌드박스**, 그보다 크면 해제된 상태다
2. 샌드박스면 **프로덕션 액세스 신청**(승인까지 하루 이틀 걸린다). 이것이 이메일 인증의
   실제 선행 조건이다 — 앱을 아무리 고쳐도 넘을 수 없다
3. 해제 확인 후 실기기에서 인증 코드 요청 → 수신 확인 → 이 티켓을 `archive/`로 옮긴다

**다음에 집는 사람이 조사할 것은 없다 — 1번을 실행하면 갈림길이 정해진다.**
