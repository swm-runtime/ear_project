...
## 처리 완료 (2026-09-08 — 실기기 종단 확인)

관리자 콘솔에서 실제로 재발행하고 앱에서 확인했다. **재진입 시 0초부터 재생된다.**
...
|---|---|---|
| `content_version` | 2 | **3** |
| `playback_progresses` | 1행(169초) | **0행** |
| `library_items` | 6건 | **6건 유지** |
| `audit_logs` | — | `content.republish` |

명세가 약속한 "저장 위치는 초기화되고 라이브러리는 유지된다"가 그대로 성립한다.

### 완료 조건 판정

- 버전 비교 분기 주석이 실제 동작만 말한다 → ✅ (PR #159)
- `contentVersion` 타입 주석에 미구현 폐기 동작이 없다 → ✅ (PR #159)
- **재발행된 콘텐츠 재진입이 0부터 (FE 코드 변경 없이)** → ✅
- 재발행되지 않은 콘텐츠는 위치 복원(회귀 없음) → ✅ (다른 콘텐츠는 이어듣기 정상)

**FE 코드 변경 없이 닫혔다** — 발행 시점 예측대로다. `issue.progress?.positionSec ?? 0` 이
서버의 `progress: null` 을 그대로 0 으로 쓴다.

### 확인 과정에서 드러난 것 — 별건으로 이관

**재발행 순간 재생 중이던 세션은 서버가 지운 위치를 앱이 곧바로 다시 쓴다.**

실측: `03:45:43` 재발행(위치 5건 삭제) → `03:48:18` 앱이 **169초를 새로 저장**했다. 그 사이
`03:47:17`·`03:47:46` 에 오디오 URL 재발급이 있었다 — `refreshAudioUrl()` 이 만료 전 URL 을
갱신하면서 **세션 위치를 그대로 유지**한 것이다. 그 함수 자체는 옳게 동작한다(갱신 시 화면이
...
이 확인은 **앱을 완전히 종료한 뒤** 해야 한다. 미니 플레이어에 세션이 살아 있으면 다시 눌러도
`start()` 가 돌지 않아 서버 응답을 쓸 기회가 없다. 처음 시도가 그래서 실패했다.
