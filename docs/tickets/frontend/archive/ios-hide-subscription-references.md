...
- **반영 날짜: 2026-09-19.** iOS **1.0.0 (7)**(main `e5a78c5`, runtimeVersion 2, production 채널)을 App Store Connect 에 올려 2026-09-19 03:27 심사에 제출했다(제출자 박수헌, 상태 "심사 대기 중"). 빌드 전 main 에서 `IS_SUBSCRIPTION_UI_ENABLED` 가 꺼진 상태인 것을 확인했다. 같은 커밋의 1.0.0 (9) 도 업로드돼 있다(예비 — 7 과 내용이 같다).
- 완료 조건 "2.1(b) 사유로 반려되지 않는다"는 심사 결과가 나와야 판정된다. 같은 사유로 다시 반려되면 새 티켓을 발행한다.
