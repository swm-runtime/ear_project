...
| Jira | [KAN-61](https://runtime364.atlassian.net/browse/KAN-61) |
...
- 코드는 운영 OTA(runtime 2)로 나가 있다. 2026-09-18 에 라이브러리·탐색 결과가 **썸네일 격자**로 바뀌면서(`changes/pending/library-thumbnail-grid.md`·`explore-results-thumbnail-grid.md`) 목록 카드의 구성이 달라졌지만, 이 티켓의 규칙은 그대로 지켜진다 — 타일은 제목·길이만 보이고 출처·저자는 상세에서만 나온다. 재생 목록 줄은 "카테고리 · 길이"다.
- **실기기 확인만 남았다**: 라이브러리·탐색(피드·필터·검색)·온보딩 담기·플레이어·재생 목록 어디에도 "참고한 자료"·저자 줄이 없고, 콘텐츠 상세에는 있는지. 확인되면 archive · KAN-61 완료.
- **마감 초과 사유**(Medium, 발행 2026-09-15): 코드는 기한 안에 끝났고 실기기 확인이 밀렸다.

## 처리 기록 (2026-09-20 — 완료, archive 로 옮긴다)

- **반영 날짜: 2026-09-20**(코드는 2026-09-17 운영 OTA 로 나갔다).
- 원본과 Jira 를 대조하다 **Jira 만 완료로 넘어가 있는 것**을 찾았다(2026-09-20). PM 이 Jira 에서 직접 닫은 것으로 확인돼, 원본을 같은 상태로 맞춘다.
