...
## 진행 기록 (2026-09-07 밤 — 죽은 링크 수정이 실기기에서 확인됐다)

Android v7에 OTA로 전달된 뒤 확인했다.

> 설정 → [이용약관] · [개인정보처리방침] → **실제 페이지가 열린다** → ✅

`ear.example.com`(존재하지 않는 도메인)으로 나가던 것이 `earcast.co.kr`로 바뀌었다. **폴백을
실값으로 둔 판단이 여기서 값을 했다** — OTA 번들은 `eas.json` env를 읽지 않으므로, 코드 폴백이
아니었다면 이 수정은 새 빌드까지 기다려야 했다.

### 남은 완료 조건 — 이메일 수신 1건

- ~~preview apk가 실서버 호출~~ ✅ / ~~구글 로그인~~ ✅ / ~~CloudFront 재생~~ ✅(DB 증거)
- Given 그 빌드 / When 이메일 인증 코드를 요청한다 / Then 실제 메일이 도착한다 — **미확인**

SES 인프라는 이미 검증됐다(2026-09-07 발송 테스트 수신 확인, 샌드박스 아님). 남은 미검증 구간은
**API 엔드포인트 + FE 배선**뿐이라 위험이 낮다. 앱에서 한 번 눌러 보면 닫힌다.

## 처리 완료 (2026-09-08 — 마지막 완료 조건 충족)

카카오 계정(이메일 `null`)으로 **앱에서 이메일을 등록해 인증 코드 메일을 받았다.**
앱 → API → SES → 수신까지 종단이 성립한다.

> Given 그 빌드 / When 이메일 인증 코드를 요청한다 / Then 실제 메일이 도착한다 → ✅
...
- preview apk 가 실서버 호출 → ✅ (2026-09-03)
- 구글 로그인 → 온보딩 진입 → ✅ (2026-09-03)
- CloudFront 서명 URL 재생 → ✅ (2026-09-07, 운영 DB 증거)
- 이메일 인증 코드 수신 → ✅ (2026-09-08)

**네 조건 모두 충족. `archive/` 로 옮긴다.**

이 티켓이 파생시킨 것들은 각자 자리에서 계속된다 — `eas.json` env 구멍(2026-09-07 수정),
카카오 채널 실값(2026-09-07 반영), SPF/DMARC(BE 티켓), 인증 메일 HTML(BE 티켓).
