...
- 2026-09-20 **완료 — archive 로 옮긴다. 반영 날짜: 2026-09-20**(코드는 2026-09-19 PR #500).
  - PM 이 개발계 앱("이어 - preview", 개발계 API)에서 실서버 동작을 확인했다 — 순서 변경이 서버에 저장되고 다시 열어도 유지된다.
  - 검증 중 찾은 계약 구멍(첫 페이지 밖 옛 항목이 맨 위로 올라옴)은 BE 가 KAN-75 로 고쳐 닫았다. FE 의 상한(50개) 조회는 그대로 둔다 — 해가 없다.
