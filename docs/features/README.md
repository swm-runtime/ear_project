...
| [explore.md](explore.md) | 추천 피드, 주제 필터, 검색, 담기 | FR-21, FR-22 |
...
| [share.md](share.md) | 공유 — OS 공유 시트, 콘텐츠 상세 연결 링크 **(P1 — MVP 미노출)**(신설 2026-08-25, `explore.md` 4.6에서 소유 이전) | FR-27 |
...
| 42 | **공유(FR-27) MVP 제외** | 더보기 시트에 공유 항목을 노출하지 않고, P1 도입 시 활성화(합의 2026-08-06). 규칙 소유는 share.md로 이전(2026-08-25 — 결정 48) | share.md |
...
| 48 | **공유 규칙 share.md 신설·소유 이전** | `explore.md` 4.6의 공유(FR-27, P1) 규칙을 `share.md`로 이전(2026-08-25). 진입점은 **네 곳**(라이브러리·탐색·플레이어 더보기 시트 + 콘텐츠 상세 상단 바 아이콘). **공유 링크는 콘텐츠 상세로 연결**(종전 "스토어 링크" 개정) — 미설치 수신자만 스토어 폴백, 디퍼드 딥링크 없음. 서버 엔드포인트·신호 기록 없음은 유지. 링크 기술 방식은 도메인 확보(earcast.co.kr, 2026-08-25)로 유니버설 링크/App Links로 기울음 — 검증 파일·리다이렉트 호스팅만 남음. spec은 `share-uiux.md` 발행(SH1~SH3 채택), api는 엔드포인트 없음으로 미작성 확정 | share.md |
| 49 | **소셜 로그인 `provider_token` 형식 확정** | 제공자마다 형식·검증 수단을 고정(2026-08-26, FE 구현 확정분 수용). **구글 = ID 토큰(JWT)** — 액세스 토큰·인가 코드가 아니며 서버가 JWKS로 서명 검증 + `aud`를 웹 클라이언트 ID와 대조한다. 카카오·네이버 = 액세스 토큰. **카카오는 토큰 정보 조회의 `app_id` 대조를 계약으로 올렸다** — 액세스 토큰에 대상 앱 정보가 없어, 이 대조가 구글·애플의 `aud` 검증에 해당하는 자리다(없으면 다른 카카오 앱 토큰으로 로그인된다). 애플 nonce 해시 인코딩은 **소문자 hex**로 명시 | auth-api 4.1, auth 4.1 |
| 50 | **안드로이드 애플 로그인 제공 — 웹 OAuth·Vercel 콜백** | 안드로이드에도 애플 버튼을 둔다(2026-08-26). "플레이스토어에는 요구 규정이 없으니 감춘다"도 검토했으나 **iOS에서 애플로 가입한 사용자의 기기 변경 경로가 막히는 것**이 결정적. 네이티브 SDK가 없어 웹 OAuth로 가며, 애플의 `form_post` 콜백은 **랜딩(Vercel)이 받고 `id_token`을 딥링크로 앱에 직송**한다 — 콜백은 비밀을 갖지 않고(code 교환 없음 → `.p8` 불필요) **검증은 NestJS가 그대로 한다**. PKCE식 핸드오프는 **nonce가 같은 일을 하므로 만들지 않는다**(저장소·스키마 변경 없음). `aud` 허용값은 번들 ID + Services ID 2개 | auth 1, auth-api 4.1, `tickets/backend/pending/apple-android-web-oauth-callback.md` |
| 51 | **추천 스코어링 3축 하이브리드 고도화** | 대본 임베딩 유사도 + 신호 기반 선호 + 메타데이터 규칙의 가중 합으로 개정(협의 2026-08-26 — "임베딩 단독으로는 부족" 피드백 반영). 스키마: `contents` 추천 메타 4종(난이도·형식·시의성·키워드, NULL 허용) + `content_embeddings`(pgvector·코사인) + `user_preference_vectors` 확장. 부여는 **별도 메타데이터 부여 파이프라인**(스킬, origin 무관 — `ai/metadata-pipeline.md` 신설)이 하고 서버는 업로드 시 저장만. 임베딩 모델·차원은 미결(확정 전 벡터 마이그레이션 금지, 스코어링은 임베딩 축 제외로 선행 가능) | drip-scheduling 4.2~4.4, domain 5.1·5.6·7.2, admin 3.1 |
| 52 | **탐험 편성 + 인기도 스무딩 + MMR 다양성** | 결정 51의 후속 고도화(협의 2026-08-27). ① 정규 2편과 별개로 **매일 1편 탐험 슬롯** — [이어 PICK] 필터 뷰는 "이런 주제는 어떠신가요?" 구획 분리, 전체 목록은 행 배지, 도착 배너는 기존 하나에 합산(카피는 uiux 확정 대기), 후보는 혼합(관심 밖 인접 주제 우선 + 관심 안 저노출), `library_items.source = discovery` 신설·`plans.daily_discovery_count`(전 티어 1), [이어 PICK] 필터·도착 배너에 포함. ② 인기도 완청률에 **베이지안 스무딩**(소표본 왜곡 방지). ③ 다양성 제약을 **MMR**(임베딩 유사도 감점)로 승격, 임베딩 없으면 이산 규칙 폴백 | drip-scheduling 4.2·4.8, library 4.1-1·4.6-1, library-api 4.1, domain 6.1·7.1·8.1, PRD FR-14 |
