...
| [content-detail.md](content-detail.md) | 콘텐츠 상세 — 더보기 시트 [상세 정보] 진입, 메타데이터 표시 전용(신설 2026-08-23) | FR-40, FR-12 |
...
| 47 | **콘텐츠 상세 화면 도입** | **세 화면(라이브러리·탐색·플레이어) 더보기 시트**에 [상세 정보] 추가, 표시 전용 상세 화면(합의 2026-08-23, FR-40 신설). 확정 표시 항목은 제목·소개·오디오 길이·발행일·출처·주제 태그 + **조건부**로 시리즈 정보(`series_id != null`)·[원문 보기](`source_url != null` — null이면 줄 생략). 청취 상태·파일 크기는 두지 않는다. 표시 항목 전부 기존 컬럼으로 성립 — 스키마 변경 없음 | content-detail 4.2·4.3-1, library 3, explore 3, player 3 |
