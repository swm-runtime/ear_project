...
| 48 | **공유 규칙 share.md 신설·소유 이전** | `explore.md` 4.6의 공유(FR-27, P1) 규칙을 `share.md`로 이전(2026-08-25). 진입점은 **네 곳**(라이브러리·탐색·플레이어 더보기 시트 + 콘텐츠 상세 상단 바 아이콘). **공유 링크는 콘텐츠 상세로 연결**(종전 "스토어 링크" 개정) — 미설치 수신자만 스토어 폴백, 디퍼드 딥링크 없음. 서버 엔드포인트·신호 기록 없음은 유지. 링크 기술 방식(유니버설 링크 vs 커스텀 스킴 병기)은 미결 | share.md |
