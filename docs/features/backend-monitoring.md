...
> 등재 2026-09-06 — 구현(2026-09-04 로그 콘솔 · 09-06 자원/DB 확장)이 먼저 있었고 문서가 뒤따른다. 어긋나면 이 문서를 기준으로 코드를 맞춘다.
...
- **구현 위치**: 콘솔은 `pipeline/apps/web/app/backend-logs/*` + `app/api/backend-{logs,status,metrics}`, 알림은 `pipeline/apps/worker/src/log-watch.ts`, 자원/DB 원천은 백엔드 `GET /admin/system-stats`(`admin-api.md` 4.9).
...
| 서버 상태 | `/backend-logs/status` | ① health 핑(타임아웃 4초) ② 로그 파이프 생존(마지막 이벤트 시각) ③ 최근 1시간 ERROR 수(상한 500) ④ 자원·DB 부하(CPU·load·메모리·연결·캐시 적중률·진행 중 쿼리, 15초 폴링) |
