...
| 대시보드 | `/backend-logs/dashboard` | 시간축 그래프 — 버킷별 요청 수(오류 적층)·응답시간 p50/p95 + **자원 이력 선 그래프**(CPU·메모리 %, DB 연결 수 — 서버의 60초 샘플 링 버퍼 최대 6시간, 재기동 시 초기화). 전 그래프 호버 툴팁·축 눈금. 헬스체크(/health)는 제외 |
