...
> **2026-08-08 반영 결과**
>
> - `common-error-handling.md` 9.8의 마지막 불릿을 제안 문구로 교체했다 — "서버 enum에 아직 없다"는 서술이 사라지고 **42개가 1:1로 일치한다**는 사실과 추가 순서(enum → 9장 표 → api 5장)만 남았다
> - 앞의 세 불릿(`retryable: true`의 범위 / 설정 화면은 고유 코드 없음 / api 문서가 없는 화면은 작성 시 등재)은 그대로 뒀다. 셋 다 여전히 유효하다
> - 9.7 프로필 표는 손대지 않았다 — `STATS_WEEK_OUT_OF_RANGE` 한 행이 이미 올바른 내용으로 등재돼 있었다
>
> **"함께 확인할 것"에 남긴 질문은 그대로 열려 있다** — 개수(42)를 문장에 박아 두는 방식은 코드가 늘 때마다 같은 수정을 부른다. 숫자를 뺄지는 문서 소유자가 판단한다.
