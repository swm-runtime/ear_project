...
- **1 — `player-api.md` 4.6 신설**(제안한 등재 위치 그대로): 요청·응답·호출 시점·주-보완
  채널 관계. 3장 표에 6번 행 추가.
- **2 — 4.3 응답에 `content_status` 추가** + `content_version` 행에 "URL 갱신으로 세션 버전을
  조용히 올리지 말 것" 경고를 함께 넣었다(재발행 실증 사례의 원인).
- **3 — `partner-control.md` 4.3 현행화**: 반영 표의 "버퍼 소진 시 중단" → 4.3 신호 중단
  (지연 상한 = 저장 주기), 5장 표현 표, 미결의 "강제 중단 여부"를 확정 처리로 닫았다.
