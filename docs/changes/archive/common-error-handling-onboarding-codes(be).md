...
| 상태 | **반영 완료** (2026-08-07, 라이브러리 통합 시점). 2026-08-05에는 아래 "보류 사유"로 보류했었다 |

> **2026-08-07 반영 결과** — 보류 사유였던 "6장 표가 존재하지 않는다"를 표 신설로 해소했다. 결정 4건은 다음과 같이 확정했다.
>
> 1. **위치** — 6장(데이터 모델) 안이 아니라 **신설 9장 "에러 코드 표"**. 코드 목록은 스키마가 아니라 계약이다. 7장(예외 상황)·8장(완료 조건)을 밀어내지 않은 것은 features 문서 전체가 같은 번호 체계를 쓰고 있어 중간 삽입이 이 문서만 어긋나게 만들기 때문이다.
> 2. **초기 범위** — 온보딩 8개가 아니라 **`spec/api/` 6개 문서 전체 + enum 기반 코드 8개, 총 40개**. 온보딩만 넣으면 라이브러리가 같은 이유로 다시 밀린다는 이 문서의 지적을 그대로 받았다.
> 3. **`CONTENT_*` 소유** — 9.2 "공통 · 콘텐츠"에 **각 한 행만** 두었다. 담기에서 `failed[]`로 전달되는 경우는 HTTP 열에 병기했다.
> 4. **중앙 표 ↔ api 5장** — **9장이 원본, api 5장은 화면분 발췌**로 확정. 6개 api 문서 5장 머리말에 명시했다.
