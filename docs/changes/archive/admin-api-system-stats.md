...
## 처리 기록 (반영 날짜: 2026-09-06 — pending 정리 2026-09-08)

**반영 자체는 발행 당일(2026-09-06~07) 이미 끝나 있었다** — `admin-api.md` 3장 목록과 4.9
`GET /admin/system-stats`(history 배열·재기동 초기화 명시 포함)로 등재돼 있다. 이 문서만
pending에 남아 있어 archive로 옮긴다. 본문의 "15초 폴링" 서술은 이후 무폴링(열 때 1회 +
새로고침, 사용자 결정 2026-09-06)으로 바뀌었고 실제 문서·구현 모두 그 기준이다.
