...
- **`runtimeVersion`은 고정 문자열이다**(`app.json`) — **`fingerprint` 정책을 쓰지 않는다.**
  ...
  **2026-09-07 실측으로 확인됨** — 고정 후 production 채널로 발행하니 **이미 배포된 Android v7이 업데이트를 받았다.** 재빌드 없이 전달됐다. 이 프로젝트에서 OTA가 실제로 도달한 첫 사례다.

  **플랫폼별로 값이 엇갈린 상태가 남아 있다.** 정책 전환 시점이 빌드마다 달라서다.

  | 빌드 | 시점 | embed된 runtimeVersion |
  |---|---|---|
  | iOS vc=3 | 2026-09-06 (정책 전환 전) | `1.0.0` |
  | Android vc=6·7 | 2026-09-06 (전환 후) | `cc07cb6e…` |

  값이 다르면 **한 번의 발행으로 양쪽에 줄 수 없다.** 다음 빌드에서 양 플랫폼을 같은 문자열로 통일해야 이 분기가 사라진다. 그때는 **앱 버전과 무관한 값**(`"1"` 같은)을 쓴다 — `1.0.0`처럼 버전을 닮은 값은 스토어 버전을 올릴 때 함께 올려야 할 것처럼 읽힌다.

  **통일 전까지는 CI가 iOS를 한 번 더 발행한다**(`eas-update.yml`의 "iOS 백필" 단계, 2026-09-08 도입). `app.json`을 레거시 지문(`1.0.0`)으로 바꿔 `--platform ios`로 발행하고 되돌린다 — `eas update`에는 `runtimeVersion`을 넘기는 플래그가 없다.

  > **이 분기의 진짜 비용은 발행 한 번이 아니라 검증의 오염이다.** 백필을 빠뜨려도 **에러가 나지 않는다** — iOS는 조용히 옛 번들로 계속 돈다. 검증하는 사람은 "고쳤다는데 안 된다"를 보고, 고친 사람은 "발행했다"를 본다. 둘 다 맞는 말이라 원인을 찾는 데 서버 로그 대조가 필요해진다(2026-09-08에 실제로 그렇게 됐다 — 회수 시각과 저장 요청 시각, `platform=` 쿼리, 새 코드에만 있는 라우트의 호출 여부까지 봐야 했다).
  >
