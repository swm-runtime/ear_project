...
| 공유 (FR-27) | `share.md` (소유 이전 2026-08-25 — 종전 `explore.md` 4.6) | **MVP 제외**(합의 2026-08-06) — 더보기 시트에 공유 항목을 노출하지 않고 P1에 활성화한다. **엔드포인트는 P1에도 없다.** OS 공유 시트는 클라이언트 조작이고(링크는 콘텐츠 상세 연결 — 개정 2026-08-25, 수신자 진입은 기존 단건 상세 조회 재사용), 시트 오픈은 신호로 기록하지 않는다(`user_signals`에 `share`가 없다 — `domain.md` 6.4) |
| 최근 검색어 | `explore.md` 4.5-4 | **엔드포인트가 없다.** `SearchHistory`는 클라이언트 로컬 전용이다(`domain.md` 13.1) |
| 콘텐츠 단건 상세 조회 (더보기 시트 [상세 정보] — 추가 2026-08-23) | `content-detail.md` · `content-detail-api.md` | **엔드포인트를 두지 않는다.** 상세 화면 도입(합의 2026-08-23 — `explore.md` 3장)은 더보기 시트에 진입 항목을 더할 뿐 탐색 API에는 영향이 없다 |
...
- **검색 초기 화면의 추천 키워드에 엔드포인트를 새로 파지 않는다**(개정 2026-08-23). `explore.md` 4.5-4가 정의한 것은 "서버가 내려주는 주제명 목록 — 관심 주제 앞배치, **4.2의 칩 정렬과 같은 서버 정렬 재사용**"이고, 그것이 정확히 **주제 칩 목록(2-2)의 응답**이다 — `name`·`is_interest`·정렬까지 이미 계약에 있다. 같은 목록을 다른 경로로 한 벌 더 내려주면 정렬 규칙이 두 곳이 되고(4.2-2가 금지한 상황), 탐색 진입 시 이미 받아 둔 응답을 버리고 다시 부르게 된다. **탭 시 동작만 다르다** — 칩 줄에서는 주제 필터(4.2), 검색 초기 화면에서는 그 `name`을 `query`로 한 즉시 검색(4.5)이다. 이 분기는 화면의 몫이며 계약은 하나다.
...
주제 칩 줄이 무엇을 보여줄지 서버에서 받아온다(`explore.md` 4.2). 탐색 탭 진입 시 한 번 조회한다. **검색 초기 화면의 추천 키워드(`explore.md` 4.5-4)도 이 응답을 재사용한다**(개정 2026-08-23 — 3장 설계 메모). 별도 조회 없이 이미 받아 둔 목록을 그대로 그리며, 추천 키워드 탭은 그 `name`을 `query`로 한 즉시 검색(4.5)이다 — 주제 필터(4.2) 호출이 아니다.
...
키워드 검색. 트림 후 2자 이상 입력에 300ms 디바운스로 호출되며, 키보드 [검색] 제출은 디바운스 없이 즉시 호출한다(`explore.md` 4.5-2). **MVP 포함**(합의 2026-08-23 — 종전 P1 표기를 폐기).
...
- **앞뒤 공백을 무시한(트림한) 뒤 2자 미만이거나 특수문자·이모지만인 쿼리는 클라이언트가 요청 자체를 보내지 않는다**("검색어를 입력해주세요" 유지 — `explore.md` 4.5-2 · 7장). 서버도 같은 기준으로 방어해 `VALIDATION_FAILED`(400)를 반환한다 — 클라이언트 필터는 UX이고 판정은 서버 방어선이다.
- **NFC 정규화·소문자 통일은 서버가 한다**(질의·저장 텍스트 양쪽, 애플리케이션 계층 — `explore.md` 4.5-5 · `domain.md` 5.1). 클라이언트는 입력을 트림 외에 가공하지 않고 그대로 보낸다 — 정규화 규칙이 양쪽에 생기면 버전마다 매칭 결과가 갈라진다.
...
- **정렬은 서버가 소유하며 정렬 파라미터를 두지 않는다.** 기준은 관련도 — 매칭 필드 가중이며, 우선순위는 **제목 > 저자 > 주제명 > 설명**, 동점은 **제목 유사도 → 인기(직전 확정 구간) → `published_at` 최신순**으로 푼다(`explore.md` 4.5-5 확정 2026-08-23 — `pg_trgm` 부분 일치·NFC 정규화 포함, 이 문서는 재정의하지 않는다). **가중 계수 값은 서버 구현이 소유한다** — 계약에 박지 않는다. 계수를 계약에 두면 랭킹 튜닝마다 문서 개정·클라이언트 검증이 따라붙는데, 클라이언트는 어차피 순서를 재배열하지 않는다(4.5-3).
- 행 모양은 4.1의 `items[]`와 **완전히 같다**(`explore.md` 4.5-3 — "피드 행과 같은 문법"). 검색 결과만 다른 행 타입을 쓰면 담기·재생 처리가 두 벌이 된다.
- **회수 콘텐츠(`contents.status != 'published'`)는 피드와 동일하게 서버 단계에서 제외한다**(`explore.md` 4.5-3 — FR-32).
- **잔여 재생 표시값을 싣지 않는다**(2장). 검색 화면은 표시를 숨긴다 — 다만 결과를 탭한 재생은 판정·팝업을 동일하게 거친다(`explore.md` 7장).
- 검색 결과 카드도 담기(4.3)·재생이 피드와 동일하게 동작하며, 더보기 시트의 분기(담기/제거·원문 보기·상세 정보)도 같다(`explore.md` 4.5-3).
- **검색 초기 화면(최근 검색어·추천 키워드)은 이 엔드포인트를 호출하지 않는다.** 최근 검색어는 클라이언트 로컬이고(1장 경계 표), 추천 키워드는 2-2 응답 재사용이다(3장 설계 메모 — 개정 2026-08-23).
- 연속 입력으로 응답 순서가 뒤바뀌면 **최신 질의의 응답만 반영한다** — 클라이언트 표시 규칙이다(`explore.md` 4.5-2). 서버는 순서를 보정하지 않는다.
...
|---|---|---|---|
| `EXPLORE_CURSOR_INVALID` | 400 | false | 커서를 버리고 첫 페이지부터 재조회. 사용자에게 노출하지 않는다. 발급 시점과 다른 `topic_ids`(4.2)·`period`(4.2-1)·`query`(4.5)도 여기에 해당한다 |
...
**검색** (`explore.md` 4.5 — 추가 2026-08-23)

```
검색창 탭 → 검색 화면 전환                    → 서버 호출 없음 (최근 검색어는 로컬,
                                              추천 키워드는 이미 받은 /explore/topics 응답 재사용)
트림 후 2자 이상 입력 (300ms 디바운스)         → GET /explore/search?query=...
  ├─ (키보드 [검색] 제출)    디바운스 없이 즉시 GET /explore/search
  ├─ (추천 키워드 탭)        그 name을 query로 즉시 GET /explore/search
  ├─ (하단 도달)            GET /explore/search?query=...&cursor=...
  └─ (빈 결과)              같은 응답의 fallback으로 관련 주제·인기 콘텐츠 표시 (추가 호출 없음)
[취소]·뒤로가기 → 피드 복귀                   → 재조회 없음 (필터·구간·스크롤 유지, 검색 상태 폐기 — explore.md 4.5-1)
```
...
| `topics` — 주제 칩 목록(4.2-2. 숨김 주제는 관심 주제여도 제외 — 결정 2026-08-11) + 검색(4.5)의 주제명 매칭 대상(`name`에는 인덱스 없음 — `domain.md` 5.1) | 4.1 |
...
- ~~"상세" 화면 처리~~ → 확정(합의 2026-08-06): 상세 화면을 두지 않는다 → **재개정(합의 2026-08-23): 상세 화면 도입 — 더보기 시트에 [상세 정보] 추가**(`explore.md` 3장 · `content-detail.md`). **이 문서에는 여전히 상세 조회 엔드포인트를 두지 않는다** — 콘텐츠 단건 상세 조회는 `content-detail-api.md`가 소유한다(1장 경계 표). 탐색 계약에 바뀌는 것은 없다.
...
- ~~공유(FR-27)의 MVP 포함 여부~~ → **확정(합의 2026-08-06): MVP 제외**(1장 경계 표). 규칙 소유는 `share.md`로 이전했다(2026-08-25 — 링크는 콘텐츠 상세 연결로 개정). P1 활성화 시에도 서버 엔드포인트는 필요 없다 — 공유 텍스트·링크 조립은 클라이언트 몫이고, 수신자의 상세 진입은 기존 단건 상세 조회를 재사용한다.
...
- ~~검색 인덱스~~ → **확정(2026-08-23): `pg_trgm`(GIN, `gin_trgm_ops`) — `domain.md` 5.1·15.1**(`explore.md` 4.5-5 — 부분 일치 + 필드 가중 랭킹. tsvector는 한국어 사전이 없어 폐기). `contents` 3필드(title·description·author_name)에 신설, `topics.name`은 인덱스 없음(주제 수십 개 — 순차 스캔). 결정 등재는 `domain.md` 15.1의 "검색 매칭·인덱스" 항목이다.
