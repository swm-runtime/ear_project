...
- 키워드 검색 (FR-22) — **MVP 포함으로 격상**(합의 2026-08-23 — `explore.md` 4.5)
...
| 5 | GET | `/explore/search` | 키워드 검색 — **MVP 포함**(합의 2026-08-23) | 필요 | |
...
- **검색(5)은 MVP 포함이다**(합의 2026-08-23 — `explore.md` 4.5. 종전 "P1 유지·검색창 비활성 노출" 합의 2026-08-06을 폐기). MVP부터 배포하고 검색창은 활성이다. **매칭·랭킹 규칙은 `explore.md` 4.5-5가 소유한다**(`pg_trgm` 부분 일치 + 필드 가중) — 이 문서는 계약만 표현하고 규칙을 재정의하지 않는다.
...
### 4.5 `GET /explore/search`

키워드 검색. 2자 이상 입력에 300ms 디바운스로 호출된다(`explore.md` 4.5). **MVP 포함**(합의 2026-08-23 — 종전 P1 표기를 폐기).
...
- **정렬은 관련도순이며 정렬 파라미터를 두지 않는다.** 매칭·랭킹 규칙은 `explore.md` 4.5-5가 소유한다(확정 2026-08-23) — `pg_trgm` 부분 일치, 필드 우선순위 제목 > 저자 > 주제명 > 설명, NFC 정규화. 이 문서는 재정의하지 않는다.
...
- **검색(4.5) 매칭은 `pg_trgm`이다**(확정 2026-08-23 — `explore.md` 4.5-5). 트라이그램 GIN 인덱스는 `domain.md` 5.1이 정의한다 — 이 문서에 인덱스를 중복 기재하지 않는다.
...
- ~~검색창 처리~~ → 확정(합의 2026-08-06): P1 유지·검색창 비활성 노출 → **재개정(합의 2026-08-23): 검색 MVP 포함 격상**(3장 설계 메모 · `explore.md` 4.5). 4.5 엔드포인트를 MVP부터 배포하고 검색창은 활성이다.
...
- ~~검색 인덱스~~ → **확정(2026-08-23): `pg_trgm`**(`explore.md` 4.5-5 — 부분 일치 + 필드 가중 랭킹. tsvector는 한국어 사전이 없어 폐기). 트라이그램 GIN 인덱스(`gin_trgm_ops`)는 `domain.md` 5.1에 반영 완료(2026-08-23) — `contents` 3필드에 신설, `topics.name`은 인덱스 없음.
