...
| 공유 (FR-27) | `explore.md` 4.6 | **엔드포인트가 없다.** OS 공유 시트는 클라이언트 조작이고, 시트 오픈은 신호로 기록하지 않는다(`user_signals`에 `share`가 없다 — `domain.md` 6.4) |
...
- **검색(5)은 P1이다.** MVP에서 빠지면 이 엔드포인트는 배포하지 않는다. 검색창 자체를 숨길지 비활성으로 둘지는 `explore.md` 미결이다.
...
- **관심 주제가 0개인 사용자**(관심사 관리에서 전부 해제 — 기존 초과 보유자의 해제 경로 등): `interest` · `topic_group` 섹션을 빼고 인기·신규만 내려준다(`explore.md` 7장).
...
**에러** — 공통 규칙(`common-error-handling.md` 4.1~4.2) 외 이 엔드포인트 고유 에러는 없다. 네트워크 실패 시 캐시 피드 노출은 클라이언트 규칙이며, 이때 **잔여 재생 표시는 노출하지 않는다**(`explore.md` 7장).
...
3. `reason = user_save`면 `user_signals`에 `action = 'save'` 적재(`domain.md` 6.4)
...
- **담기 UI의 pages 개정 필요** — FE 확정(2026-08-06): **행에는 담기 버튼을 두지 않고 더보기 시트가 [라이브러리에 담기]/[라이브러리에서 제거]를 소유한다.** 담긴 행은 "담김" 표시만 붙는다. 이 구성은 `explore.md` 4.1("담기 버튼 비활성")·4.3("이미 담긴 콘텐츠를 다시 탭하면 해제")의 버튼 전제 서술과 어긋나므로 **pages 개정이 필요하다**(`changes/` 절차).
    - 제거는 `library`가 있는 모든 행에 허용한다(출처 무관). 드립 적립분을 탐색에서 제거하면 `reason = unsave`로, 라이브러리에서 삭제하면 `library_delete`로 적재된다 — `reason`은 운영값이라 실질 차이는 없다(`domain.md` 7.1).
- **탐색 재생 자동 적립의 문서 간 충돌** — `explore.md` 4.4는 "탐색에서 재생한 콘텐츠는 자동 적립(`source = save`)"이라 하고, `library-api.md` 4.4 설계 메모는 "재생이 담기를 유발하지 않는다"고 한다. 이 문서는 4.6처럼 **클라이언트가 재생 개시 후 `save(reason=auto_play)`를 호출하는 방식**으로 두 문서를 함께 만족시켰다. 다만 `library-api.md`의 해당 메모가 `explore.md`를 근거로 인용하고 있어 문구 정리가 필요하다.
...
- **인기 섹션의 표본 부족** — 서비스 첫 주에는 직전 확정 구간(`is_final = true`) 데이터 자체가 없다. 이때 인기 섹션을 숨길지, 진행 중 구간으로 임시 대체할지 결정이 필요하다.
