...
### 4.4 `GET /notices` — 공지사항 목록

설정 > 정보 > [공지사항]의 목록(신설 2026-09-17 — `changes/archive/notice-screen-spec.md` C, KAN-67). 운영 공지 게시판이며 **점검 공지(`domain.md` 13.3)와 별개**다.

| 쿼리 | 타입 | 설명 |
|---|---|---|
| `cursor` | string | 선택. 직전 응답의 `next_cursor`. 불투명 문자열 |
...
- **발행된 공지만** — `published_at <= 서버 시각`이고 삭제되지 않은 것. 초안(`published_at` 없음)·예약(미래 시각)은 나오지 않는다. **클라이언트는 기기 시각으로 거르지 않는다.**
- 정렬 `is_pinned DESC, published_at DESC, id DESC` — 고정 공지가 먼저, 고정끼리도 최신순. 커서는 이 정렬의 keyset 위치다.
- **본문(`body`)은 싣지 않는다** — 목록은 가볍게, 본문은 상세에서.
- 커서가 깨졌으면 400 `NOTICE_CURSOR_INVALID` — 커서를 버리고 첫 페이지부터 다시 조회한다.

### 4.5 `GET /notices/:notice_id` — 공지사항 상세
...
- ~~공지사항의 형태~~ → **형태는 확정(2026-08-06): 인앱 화면**(`settings.md` 4.1). ~~목록 API~~ → **확정(2026-09-17): 4.4·4.5**(`changes/archive/notice-screen-spec.md` C, KAN-67).
