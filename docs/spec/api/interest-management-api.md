...
- **숨겨진 주제(`is_visible = false`)의 활성 관심사는 응답에서 제외한다.** 칩으로 그릴 수 없는 선택지를 내려줘도 화면이 할 수 있는 일이 없고, 편집·저장의 대상 범위(4.3의 diff 범위)와 응답 범위가 같아야 "조회한 것을 고쳐서 되돌려보낸다"는 계약이 성립한다. **행 자체는 서버에 그대로 남아 있으며**(`interest-management.md` 7장 — 관리자가 다시 노출하면 되살아난다), 4.3이 이 행을 건드리지 않는다.
...
- **diff의 범위는 노출 중인 주제로 한정한다.** 관리자가 숨긴 주제의 활성 행은 요청 목록에 없어도 해제로 처리하지 않는다 — 클라이언트는 그 주제를 화면에 그릴 수 없어 목록에 담을 방법이 없고, 여기서 해제해버리면 `interest-management.md` 7장의 "관리자가 다시 노출하면 그대로 되살아난다"가 성립하지 않는다. 게다가 `is_user_removed = true`가 서면 사용자가 하지 않은 해제가 자동 확장 영구 제외로 기록된다.
...
GET /users/me/interests ┘        + 현재 선택(source 원값)
   ↓ 칩 편집 (0개 상태 허용, N/3 표시, 4번째 탭은 토스트 — 전부 클라이언트)
[저장] 탭
   ├─ 추가만 있음        → 팝업 없이 바로 ↓
