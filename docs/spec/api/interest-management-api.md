...
**에러** — `ONBOARDING_NOT_COMPLETED`(409, `onboarding-api.md` 5장과 같은 코드 — 반영 2026-09-15): 온보딩을 마치지 않은 계정. 관리 화면 진입 전이라 정상 클라이언트는 도달하지 않는다(5장). 조회 실패(5xx)는 전체 화면 에러 + [다시 시도](`common-error-handling.md` 4.3).
...
| `ONBOARDING_NOT_COMPLETED` | 409 | 온보딩을 마치지 않은 계정(반영 2026-09-15). 관리 화면 진입 전이라 정상 클라이언트는 도달하지 않는다 |
...
- **온보딩 미완료 계정은 4.2·4.3 모두 `ONBOARDING_NOT_COMPLETED`(409, `onboarding-api.md` 5장과 같은 코드)로 거부한다**(반영 2026-09-15). 두 저장 경로(온보딩·관리)가 같은 계정에 동시에 열려 있으면 출처가 다른 행이 겹쳐 상한이 우회되기 때문이다 — 온보딩 저장은 `onboarding` 출처 행만 내리고 관리 저장은 `manual` 출처로 넣어, 미완료 계정이 둘을 번갈아 쓰면 활성 6개가 되고 `max(3, 보유 개수)` 규칙이 그 6개를 영구 허용한다. 위의 "재사용하지 않는다" 규칙은 **상한 판정** 코드에 대한 것이고, 이 코드는 상한이 아니라 전제 조건이라 충돌하지 않는다.
