...
| provider_token | string | 필수 | 서버가 **반드시 검증**한다. 셋은 제공자 API 호출로, **`apple`은 identity token(JWT)의 애플 공개키 서명 검증 + `iss`·`aud`·`exp`·nonce 대조**로 한다 — 검증 수단이 다르다 |
| device_id | string | 필수 | 푸시 토큰 매핑용 |
| nonce | string | **`apple`만 필수** | 클라이언트가 애플 인가 요청에 실은 **원본** nonce. 다른 제공자는 무시한다 |

**`nonce` — 애플 재전송 공격 차단**

클라이언트는 요청마다 임의의 nonce를 만들어 **해시(SHA-256)를 애플 인가 요청에 싣고, 원본을 이 API에 함께 보낸다.** 애플은 받은 해시를 identity token의 `nonce` 클레임에 담아 돌려주므로, 서버가 원본을 해시해 대조하면 **그 토큰이 이번 요청을 위해 발급된 것인지** 확인할 수 있다.
