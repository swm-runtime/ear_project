...
- **파이프라인 SSO(12)는 앱 클라이언트가 부르지 않는다.** 파이프라인 웹 **서버**만 호출한다 — 어서션 서명에 쓰는 공유 비밀을 브라우저가 알아서는 안 되기 때문이다. 앱은 1~11만 쓴다.
...
{
  "provider": "kakao",
  "provider_token": "<제공자별 형식 — 아래 표>",
  "device_id": "<기기 식별자>",
  "nonce": "<애플 전용 — 인가 요청에 실은 원본 nonce>"
}
```

| 필드 | 타입 | 필수 | 비고 |
| --- | --- | --- | --- |
| provider | enum — `kakao` · `naver` · `google` · `apple` | 필수 | **`apple`은 iOS 심사 요건이다**(`auth.md` 1 — App Store 심사 가이드라인 4.8) |
| provider_token | string | 필수 | **제공자마다 형식과 검증 수단이 다르다** — 아래 "provider_token — 제공자별 형식" 참조. 어느 쪽이든 서버가 **반드시 검증**한다 |
| device_id | string | 필수 | 푸시 토큰 매핑용 |
| nonce | string | **`apple`만 필수** | 클라이언트가 애플 인가 요청에 실은 **원본** nonce. 다른 제공자는 무시한다 |

**`provider_token` — 제공자별 형식**

클라이언트가 보내는 값이 제공자마다 다르다. **문서가 "인증 코드 또는 액세스 토큰"으로 열려 있으면 서버 검증이 클라이언트가 실제 보내는 값과 어긋난다** — 확정 형식으로 고정한다(FE 구현 확정 2026-08-26).

| provider | provider_token | 서버의 검증 수단 |
| --- | --- | --- |
| `google` | **ID 토큰(JWT)** | 구글 공개키(JWKS)로 서명 검증 + `iss`·`exp` 확인 + `aud`를 **우리 웹 클라이언트 ID**와 대조. **액세스 토큰도 인증 코드도 아니다** — 제공자 API를 왕복하지 않는다 |
| `kakao` | 액세스 토큰 | 카카오 API 호출. **토큰 정보 조회의 `app_id`를 우리 앱 ID와 대조**한 뒤 프로필을 받는다 |
| `naver` | 액세스 토큰 | 네이버 프로필 API 호출 |
| `apple` | identity token(JWT) + `nonce`(원본) | 애플 공개키로 서명 검증 + `iss`·`aud`·`exp`·nonce 대조. **`aud` 허용값이 둘이다** — 아래 참조 |

- **`google`·`apple`은 토큰 자체로 검증이 끝난다.** 그래서 `aud`가 "우리 앱을 향해 발급된 토큰인가"를 가르는 유일한 근거다 — 확인하지 않으면 다른 앱을 향해 발급된, 서명은 정상인 토큰으로 우리 계정에 로그인할 수 있다.
- **`kakao`·`naver`는 제공자 API 왕복으로 검증한다.** 액세스 토큰에는 대상 앱 정보가 실려 있지 않으므로, **카카오는 `app_id` 대조가 `aud` 검증에 해당하는 자리**다.
- **네이버는 앱 식별 호출이 없다. 대신 네이버 `id`가 애플리케이션별 pairwise 값이라는 성질에 기댄다**(개정 2026-09-08). 다른 앱에서 발급된 토큰으로 프로필 API를 부르면 **그 앱 네임스페이스의 id**가 돌아오므로 `uq_users_provider_provider_user_id`에 걸린 우리 사용자 행과 일치할 수 없다. 근거: 네이버 프로필 API 명세 5장("같은 네이버 회원이라도 … 애플리케이션이 다르면 id값이 다르다") · OIDC discovery의 `subject_types_supported: ["pairwise"]`.
  - **`/v1/nid/verify`는 앱 식별이 아니다** — 응답에 `client_id`가 없어 유효성·허용 범위 확인에 그친다.
  - **`aud` 검증 수단 자체는 존재한다** — 네이버 OIDC(`scope=openid` → `id_token`, RS256 + JWKS)가 구글·애플과 같은 모양이다. 지금 쓰지 않는 것은 네이티브 SDK가 액세스 토큰만 돌려주기 때문이고, 옮기려면 `sub`가 현재 `id`와 다른 값이라 **`provider_user_id` 마이그레이션**이 따라온다(미결).
  - **이 성질이 깨지면 남의 앱 토큰이 곧 계정 탈취다.** 그래서 근거를 여기 적어 둔다 — 없던 동안 실제로 "네이버만 검증이 없으니 탈취 가능"으로 오독된 적이 있다(2026-09-08 점검).
- 검증에 쓰는 식별자(구글 웹 클라이언트 ID·카카오 앱 ID·애플 `aud`)는 비밀값이 아니지만 **검증에 반드시 필요**하므로 배포 환경변수로 둔다(`backend/.env.example`).

**`nonce` — 애플 재전송 공격 차단**

클라이언트는 요청마다 임의의 nonce를 만들어 **해시(SHA-256, 소문자 hex)를 애플 인가 요청에 싣고, 원본을 이 API에 함께 보낸다.** 애플은 받은 해시를 identity token의 `nonce` 클레임에 담아 돌려주므로, 서버가 원본을 해시해 대조하면 **그 토큰이 이번 요청을 위해 발급된 것인지** 확인할 수 있다.

- **원본을 애플에 보내지 않는다.** 애플에 가는 것은 해시, 서버에 오는 것은 원본이다.
- **해시 인코딩은 소문자 hex다.** 애플 네이티브 예제와 `expo-crypto`의 기본 출력이 hex이며, 클라이언트가 그 값을 그대로 싣는다. **서버가 원본을 해시해 대조할 때 같은 인코딩을 써야 한다** — 인코딩이 어긋나면 대조가 항상 실패한다.
- **토큰에 `nonce` 클레임이 있는데 요청에 `nonce`가 없으면 거부한다.** 우리 클라이언트는 항상 보내므로, 없다는 것은 토큰이 다른 경로로 흘러들어왔다는 뜻이다.
- 애플은 **제공자 API 호출 없이 토큰만으로 검증이 끝나므로**, `aud`와 nonce가 "우리 앱의 이번 로그인을 위한 토큰인가"를 가르는 유일한 근거다.
- **동의 값을 여기에 싣지 않는다.** 동의는 로그인 성공 이후 별도 화면에서 받는다(`auth.md` 4.1).
- 클라이언트가 보낸 프로필 정보(이메일·이름·소셜 ID)를 **그대로 신뢰하지 않는다**(`architecture.md` 9.1).

**`apple` — `aud` 허용값 2개 (플랫폼별 발급 경로가 다르다)**

애플 로그인은 플랫폼마다 토큰을 받아오는 경로가 달라 **`aud`가 다르게 실려 온다.** 서버는 **둘 다 유효한 `aud`로 받고, 그 외는 거부**한다(README 결정 50).

| 플랫폼 | 경로 | `aud` |
| --- | --- | --- |
| iOS | 애플 네이티브 SDK | **앱 Bundle ID** (`APPLE_CLIENT_ID`) |
| Android | 애플 웹 OAuth | **Services ID** (`APPLE_SERVICES_ID`) |

안드로이드에는 애플 네이티브 SDK가 없어 브라우저를 한 번 거친다. 애플은 등록된 HTTPS Return URL로만 결과를 보내고 `scope`에 name·email이 있으면 `form_post`(POST)를 강제하므로, **랜딩(`earcast.co.kr`)의 콜백이 그 POST를 받아 앱으로 되돌린다.**

- **콜백은 이 API 서버가 아니다.** 토큰을 검증하지 않고 비밀값도 갖지 않는 중계기이며, 앱이 받은 `id_token`을 이 엔드포인트로 보내야 비로소 검증된다. **계약상 두 플랫폼의 요청은 완전히 같다** — `provider: "apple"`, `provider_token`, `nonce`.
- **nonce 규칙은 두 플로우가 동일하다.** 원본은 앱이 보관해 이 API에만 싣고, 애플 인가 요청에는 해시만 싣는다. **원본이 브라우저·콜백을 거치면 안 된다** — 거치는 순간 딥링크 탈취가 곧 계정 탈취가 된다.
...
**애플은 두 가지가 다르다**(`auth.md` 4.1).

- **이메일·이름은 최초 인가 때 한 번만 내려온다.** 재로그인 응답에는 오지 않으므로 **첫 응답에서 반드시 저장한다.** 이후 응답의 빈 값으로 기존 값을 덮어쓰지 않는다 — 놓치면 사용자가 애플 설정에서 앱 연동을 해제하기 전까지 다시 받을 수 없다.
- **"이메일 가리기"를 고르면 릴레이 주소(`...@privaterelay.appleid.com`)가 온다.** 값이 온전하고 발송도 되므로 **저장하고 `is_email_verified = true`로 둔다** — 발송 불가인 카카오 마스킹 주소와 다르다. 다만 사용자가 언제든 전달을 끊을 수 있어 도달을 영구히 보장하지는 않는다.
...
**Request** — `Idempotency-Key`를 받지만 **응답을 캐시하지 않는다**(개정 2026-09-08). 헤더는 클라이언트의 5xx 자동 재시도 조건이라 계속 보낸다.

- **응답에 `refresh_token` 원문이 실려 있어 캐시할 수 없다.** 저장하면 `idempotency_keys.response_body`에 **30일짜리 자격증명이 평문으로** 남는다 — `sessions`가 해시만 두는 이유([3.3](#33-sessions))를 그 한 테이블이 무력화한다.
- **캐시가 없어도 계정은 하나다.** 순차 재시도는 서버가 `provider + provider_user_id`로 기존 계정을 찾아 그대로 돌려주고, 동시 요청은 `uq_users_provider_provider_user_id`가 막는다(서버가 유니크 위반을 흡수한다 — `architecture.md` 8.4).
- **차이는 재시도가 받는 토큰 값뿐이다.** 저장된 첫 응답이 아니라 새 세션이 발급된다. 같은 계정이므로 클라이언트 동작에는 영향이 없다.
...
| `EMAIL_ALREADY_REGISTERED` | 409 | — | **계정에 인증까지 끝난 주소가 이미 있다.** 보낸 주소가 그 값이든 다른 값이든 같다(개정 2026-09-08). 코드를 보내지 않으며 **발송 횟수를 소모하지 않는다** |
...
- **판정 기준은 계정의 `is_email_verified`이지 보낸 주소가 아니다**(개정 2026-09-08). 인증이 끝난 계정이 **다른 주소**를 보내면 그것도 409다 — `auth.md` 4.4가 "인증이 끝난 이메일은 앱에서 변경할 수 없다"로 확정했고, 새 주소로 코드 인증을 통과하면 변경을 허용하는 안은 **검토 후 기각**됐다. 화면이 진입점을 감추는 것은 안내이지 판정이 아니므로 서버가 막는다.
...
### 4.12 `POST /auth/pipeline-login` — 파이프라인 웹 SSO

발행 콘솔(파이프라인 웹 `/publish`)의 **두 번째 로그인을 없애는 서버 간 신뢰**다. 운영자는 이미 Supabase 팀원 로그인을 거쳤는데, 제품 관리 API를 쓰려고 다시 구글 로그인을 하던 마찰을 없앤다(`admin.md` 2장).

```jsonc
// 요청
{
  "assertion": "<HS256 JWT>",   // 2000자 이내
  "device_id": "..."            // 200자 이내
}
```

`assertion`의 페이로드: `typ = "pipeline_sso"` · `email` · `exp ≤ 60초`. **파이프라인 웹 서버가 `EAR_SSO_SECRET`으로 서명하고, 제품 서버가 `PIPELINE_SSO_SECRET`으로 검증한다**(같은 값이어야 한다).

```jsonc
// 200 — 일반 세션과 동일한 토큰 쌍이다. 갱신(4.3)·로그아웃(4.4)을 그대로 공유한다
...
| 403 | `FORBIDDEN` | 그 이메일의 **관리자 계정이 없다**. 비관리자 계정도 여기 포함된다 |
| 503 | `AUTH_PROVIDER_UNAVAILABLE` | 서버에 `PIPELINE_SSO_SECRET`이 설정돼 있지 않다 — 이 경로가 비활성 상태다 |

- **계정 체계가 아니라 로그인 수단이다.** 계정은 여전히 소셜 가입으로 만들어지고 승격은 사람이 DB에서 한다(`users.role`). 이 경로는 **이미 존재하는 관리자 계정에만** 세션을 내주며 새 자격 증명을 만들지 않는다 — `auth.md` 4.1의 "자체 아이디/비밀번호를 만들지 않는다"와 충돌하지 않는다.
- **비관리자를 403으로 막는 이유**: 이 경로의 용도가 관리 콘솔뿐이라 최소 권한으로 좁힌다. 일반 사용자에게 세션을 내주면 앱 로그인 경로가 하나 더 생긴다.
...
**이 표는 `common-error-handling.md` 9장 중앙 표의 인증·계정 발췌(9.3)다** — 두 곳이 어긋나면 9장이 기준이다. 추가·변경 시 `architecture.md` 7.5에 따라 enum 한 곳에서 관리하고 **9장 표를 먼저 갱신한 뒤** 이 표를 맞춘다. 이미 배포된 코드의 의미를 바꾸지 않는다.

401·429·5xx 등 도메인 무관 기반 코드는 9.1을 따르며 이 표가 재정의하지 않는다.
...
- **계정 단위 발송 상한(1시간 20회 / 하루 50회)을 발송 전에 판정한다**(확정 2026-08-10 — `auth.md` 4.5). 클라이언트 비노출 — 초과 시 일반 오류로 거절하며 전용 코드·남은 시간을 내려주지 않는다(9장 참조).
...
- ~~계정 단위 발송 상한(백스톱) — P0~~ → **해소(2026-08-10)** — **최근 1시간 20회 / 최근 24시간 50회**를 서버가 발송 전에 함께 판정한다(`auth.md` 4.5 · `domain.md` 3.7 — 발송 이력 슬라이딩 집계, 컬럼 추가 없음). **전용 에러 코드는 신설하지 않는다** — 클라이언트 비노출 원칙이라 초과 시 일반 오류로 거절하고 남은 횟수·해제 시각을 응답에 담지 않는다(정상 사용자는 주소 단위 5회·쿨다운 30초에 먼저 걸려 도달 불가, 노출하면 우회 힌트만 된다). **클라이언트 대응 작업 없음** — 4.8 계약은 변하지 않는다.
