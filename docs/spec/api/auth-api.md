...
| `apple` | identity token(JWT) + `nonce`(원본) | 애플 공개키로 서명 검증 + `iss`·`aud`·`exp`·nonce 대조 |
...
- 애플은 **제공자 API 호출 없이 토큰만으로 검증이 끝나므로**, `aud`(앱 Bundle ID)와 nonce가 "우리 앱의 이번 로그인을 위한 토큰인가"를 가르는 유일한 근거다.
