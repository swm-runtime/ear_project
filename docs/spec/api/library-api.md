...
|---|---|
| *(미선택)* | 출처를 가리지 않는다 |
| `drip` | `source IN ('drip', 'discovery')` — **탐험 편성분을 포함한다**(개정 2026-08-27 — `library.md` 4.1-1) |
...
| 필드 | 의미 |
|---|---|
| `source` | `drip` / `save` / `onboarding` / `discovery` — `library_items.source` 원값. 배지·타이틀 매핑은 화면이 한다. **`discovery`는 탐험 편성이며 "이런 주제는 어떠신가요?" 구분 표시의 판정 재료다**(신설 2026-08-27 — `library.md` 4.6-1, 새 필드를 두지 않는다) |
...
- **비교 대상은 `source IN ('drip', 'discovery')`인 항목이다**(`library.md` 4.6 — 배너가 알리는 사건은 편성 도착이고, 탐험 편도 이어가 보내준 것이다. 개정 2026-08-27). 판정 재료는 이 응답의 `source`이며 **새 필드를 두지 않는다.**

| 항목 | 규칙 |
|---|---|
| 기준값 | 직전에 노출한 목록에서 `source IN ('drip', 'discovery')`인 항목의 **최대 `added_at`** |
| 배너 수 | 재조회 결과 중 `source IN ('drip', 'discovery')`이면서 `added_at`이 기준값보다 큰 항목 수 |
...
4. 순서가 없던 항목은 이번 목록에 들어왔을 때만 순서를 얻는다. 새로 담긴 항목이 맨 위에 오는 것은 저장이 아니라 **조회 규칙**(`NULLS FIRST`)이 만든다.
...
- 클라이언트 이관: 기기 저장 시절의 순서(`player.queue_order`)는 첫 호출 때 한 번 올리면 된다 — 서버 규칙(새 항목 맨 위·미지정은 뒤)이 FE `applyQueueOrder`와 같다.
