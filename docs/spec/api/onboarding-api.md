...
| `is_fallback` | **레거시**(개정 2026-09-09). 서버가 폴백 세트를 만들지 않으므로 항상 `false`다. 계약 호환을 위해 필드만 남는다 |
...
- **노출 주제가 0건이면 `items: []`를 200으로 반환하고 서버가 운영 알림을 발생시킨다**(`onboarding.md` 7, 개정 2026-09-09 — 종전 폴백 세트 제거). 클라이언트는 재시도 화면(O6 변형)을 그린다 — 하드코딩한 주제를 그리면 그 선택을 `user_interests`(`topic_id` FK)에 저장할 수 없어 1단계가 통째로 막힌다.
...
**이 표는 `common-error-handling.md` 9장 중앙 표의 온보딩 발췌(9.4)에 공용 코드를 더한 것이다** — 두 곳이 어긋나면 9장이 기준이다. 추가·변경 시 `architecture.md` 7.5에 따라 enum 한 곳에서 관리하고 **9장 표를 먼저 갱신한 뒤** 이 표를 맞춘다. 이미 배포된 코드의 의미를 바꾸지 않는다.
