...
현재 제공하는 것은 **임베딩 API 하나**다. 이 서버에는 임베딩처럼 **요청-응답형 단발 추론 API**만 둔다(추후 TTS 등). **대본 생성·QA·비평은 여기에 추가하지 않는다** — 같은 인스턴스에서 도는 콘텐츠 파이프라인의 워커(`pipeline/apps/worker`, `docs/ai/spec/10-webapp.md`)가 담당한다(현재는 팀원 구독의 `claude -p` 실행기, 이후 API 실행기 — spec/10 3.1). "AI 서버"는 이 두 프로세스를 함께 올리는 **EC2 배포 단위**를 뜻한다(정렬 2026-09-01).
