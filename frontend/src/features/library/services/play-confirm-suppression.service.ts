...
export const hydrateSuppressedServiceDate = async (): Promise<void> => {
  const stored = await secureStorage.get(STORAGE_KEYS.LIBRARY_PLAY_CONFIRM_SUPPRESSED_DATE);
  useLibraryStore.getState().setSuppressedServiceDate(stored);
};
...
export const suppressPlayConfirmForToday = async (serviceDate: string): Promise<void> => {
  useLibraryStore.getState().setSuppressedServiceDate(serviceDate);
  await secureStorage.set(STORAGE_KEYS.LIBRARY_PLAY_CONFIRM_SUPPRESSED_DATE, serviceDate);
};
