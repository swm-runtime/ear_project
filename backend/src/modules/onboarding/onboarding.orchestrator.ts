...
export class OnboardingOrchestrator {
  ...
  private async selectDiscoveryContents(
    ...
    /**
     * **표본이 부족해도 선정 기준을 바꾸지 않는다**(`onboarding.md` 91행).
     *
     * 종전에는 월간 재생 합계가 30건 미만이면 랜덤 3건 + 섹션 제목 교체로 빠졌는데,
     * 그 규칙은 폐기됐다(`features/README.md` 결정 12 — 2026-08-06). 게다가
     * `content_stats` 집계 배치가 없어 합계가 항상 0이었으므로 **모든 사용자가 그 폐기된
     * 경로만 타고 있었다.**
     *
     * 순위가 비어 있으면 아래 정렬이 원래 후보 순서를 유지한다 — 인기 데이터가 쌓이기
     * 전에도 같은 기준으로 뽑고, 쌓이면 그대로 반영된다.
     */
    const rankedIds =
