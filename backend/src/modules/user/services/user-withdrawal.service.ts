...
export class UserWithdrawalService {
   ...
   * `user_settings` · `user_preference_vectors` · `source_link_clicks`
   *
   * **명시적으로 지우는 것** — FK가 없거나(`idempotency_keys`는 `owner_key` 스코프)
   * 아카이브 이관 순서가 있어야 하는 것(`subscriptions` · `consents`)이다.
   *
   * domain.md 12.3의 즉시 파기 대상 중 `notification_logs`(1786700000000-AddMissingDomainTables)와
   * `purchase_intents`(1787800000000-AddPurchaseIntentsAndStoreNotificationLogs)도 `user_id` FK의
   * ON DELETE CASCADE로 함께 지워진다. `store_notification_logs`는 개인 식별자가 없어 그대로 둔다.
   */
