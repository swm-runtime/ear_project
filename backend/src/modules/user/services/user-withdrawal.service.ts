...
export class UserWithdrawalService {
   ...
   *
   * domain.md 12.3의 즉시 파기 대상 중 `notification_logs`는 `user_id` FK의 ON DELETE CASCADE로
   * 함께 지워진다(1786700000000-AddMissingDomainTables). **아직 테이블이 없는** `purchase_intents`도
   * 도입 시 같은 방식으로 CASCADE를 걸면 여기에 손댈 필요가 없다.
   */
