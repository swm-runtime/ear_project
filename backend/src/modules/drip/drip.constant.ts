...
export const META_ITEM_WEIGHTS = {
   ...
   * 조금 크고 주제 적합도(0.30)보다 작다 — 메타 축이 최종의 0.35이므로 전체의 약 5%. 동점 구간의
 ...
 *
 * 재실행이 안전한 근거는 `library_items` 유니크가 **아니다** — 유니크는 같은 콘텐츠의 중복만 막고,
 * 후보 SQL이 이미 준 콘텐츠를 빼기 때문에 두 번 돌면 **다른 2+1편이 또 간다**(2026-09-26 감사에서
 * 정정). 사용자 단위 멱등은 `planForUser`의 `already_placed` 스킵(오늘 서비스 날짜에 편성분이 있으면
 * 건너뜀)이 보장한다.
 *
...
/**
 * 사용자 한 명의 편성이 던지면 한 번만 다시 시도한다(`drip-scheduling.md` 2장 "편성 실패 건 재처리").
 * 커넥션 순간 끊김 같은 일시 오류가 대상이고, 두 번째도 실패하면 `failed_count`로 남긴다.
 * 적립 뒤에 던진 경우는 `already_placed` 스킵이 이중 적립을 막는다.
 */
export const DRIP_BATCH_USER_RETRY_DELAY_MS = 1_000;
