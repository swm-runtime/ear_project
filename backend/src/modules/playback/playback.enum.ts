...
export enum PlayEntryPoint {
  ...
  PLAYER = 'player',
  /**
   * 공유 링크 수신으로 진입한 콘텐츠 상세의 [재생](신설 2026-08-25 — FR-27 P1, `share.md` 4.3).
   *
   * 일반 경로(더보기 > 상세 정보)의 상세는 **원 화면 값을 유지 전달**하므로(content-detail-api.md
   * 4.2) 이 값을 쓰지 않는다 — 원 화면이 없는 공유 유입만의 값이다.
   */
  SHARE = 'share',
}
