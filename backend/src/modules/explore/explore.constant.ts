 ...
 *
 * 인기·신선도 순 상위 이만큼을 읽어 **선호도 캐시로 재정렬한다.** 전체를 정렬하지 않는
 * 이유는 점수가 콘텐츠×주제 조인이라 SQL 한 문장으로 표현하려면 가중치 테이블을 DB로
 * 밀어 넣어야 하는데, 그 값은 편성 배치가 사용자별로 접어 둔 파생 캐시
 * (`user_preference_vectors.topic_weights` — domain.md 7.2)라 조인 대상이 아니다.
 *
 * **한계**: 인기·신선도 상위 밖에 있는 콘텐츠는 주제 가중치가 높아도 올라오지 못한다.
