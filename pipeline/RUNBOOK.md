...
- **자동화 스위치** (2026-09-23, 0023): 설정 → 자동화 — "후보 자동 승인"·"비평 뒤 발행 준비 자동". 끄면 그 지점부터 종전대로 사람이 누른다. **"서버 워커가 AI 작업을 집는다"**(2026-09-26)를 끄면 서버는 io 작업만 하고 AI 작업은 노트북 Claude 워커가 집는다 — 군집화를 Claude 로 돌릴 때 끄고, 끝나면 다시 켠다(끈 채 두면 초안 자동화도 멈춘다). 알림은 편마다가 아니라 **큐가 다 비었을 때 한 번** — 검수 대기 편 목록·검토 필요·초안 실패 복귀·자동 반려·연쇄 실패를 요약해 `SLACK_ERROR_WEBHOOK_URL` 로 보낸다(서버 워커만, `digest.ts`).
