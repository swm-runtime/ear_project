...
표 한 개: \`| C## | 주장 (한국어, 한 문장) | 발췌 ID | 유형 | 구간 | 저명 | 의견 |\`. 유형은 수치·인용·고유명사·인과·정의·실천 중 하나. 구간 = 이 claim 을 쓰는 본문 구간 번호(같은 소스의 claims 는 전부 같은 구간, 축 소스만 둘) · 저명 = 주체가 소개 없이 통하는 인물·기관이면 ○ · 의견 = 특정인의 의견·해석·예측·논쟁적 주장이면 ○. 귀속 등급(이름·익명·없음)은 이 셋에서 대본 단계가 도출한다(규칙 20).
...
- 구간별 앵커: #1 S? · #2 S? … (그 구간에서 claims 3개 이상을 소비하는 소스 — 대본에서 이름이 불리는 소스. 구간당 최대 둘)
...
- **귀속은 claims 의 \`귀속\` 열대로** (guidelines 규칙 20~22, full-v7 2026-09-15):
  · \`없음\` 주장은 **해설자가 자기 말로 설명한다** — "이 글은 ~라고 해요", "~에 따르면", "~라는 얘기가 있어요"로 감싸지 않는다. 사실 검증은 claims 로 하므로 출처를 말하지 않아도 된다.
  · \`익명\` 주장은 익명으로 귀속한다("한 조사에서는", "어느 연구자는", "이렇게 보는 쪽도 있어요") — 수치가 처음 나오는 문장 앞에. 이름을 부르지 않는다.
  · \`이름\` 주장만 매체·저자·기관 이름을 부른다 — **소스당 이름은 하나**(저자 또는 매체 중 하나), 첫 등장에서 "~라는 곳/사람" 한마디로 소개(저명 주체는 소개 없이 이름만). **이름의 형태는 첫 등장 그대로** — 풀네임으로 시작했으면 끝까지 풀네임, 성만 따로 부르지 않는다.
  · **소스는 claims 의 \`구간\` 열에 적힌 구간에서만 쓴다** — 다른 구간에서 그 소스를 되부르지 않는다(축 소스만 도입·착지). 같은 구간 안에서는 처음 한 문장으로 소개하고 이후 내용으로 잇는다. **마무리에는 claims 를 쓰지 않는다** — 무귀속 요약, 이름 재호명 없음, 축을 회수한 문장에서 끝난다(사족 없음).
  · 자기 점검: 이름이 나오는 소스가 본문 구간 수 + 1 을 넘지 않는가 · \`이름\` 등급이 없는 소스의 이름을 부르지 않았는가 · 귀속 표현이 든 해설 턴이 다섯 턴 연속이 아닌가. L0 가 잡는다.
...
- **귀속·소스 (규칙 20~22)**: 소스를 claims 의 구간 밖에서 쓰지 않았는가. 이름은 \`이름\` 등급 소스에만, 소스당 하나, 형태는 첫 등장 그대로인가. 마무리에 claims·이름이 없는가. 퍼센트/퍼센트포인트 표기가 한 대본 안에서 같은가(규칙 24).
...
- **귀속 등급(이름·익명·없음)은 당신이 고르지 않는다 — 워커가 section·type·notable·opinion 에서 도출한다** (guidelines 규칙 20, full-v7): 한 구간 안에서 claims 를 **3개 이상** 가진 소스가 그 구간의 **앵커**(구간당 최대 둘)이고 앵커의 claims 만 이름 귀속이다. 앵커 아닌 소스의 수치·인용·opinion 은 익명 귀속(notable 이면 이름), 그 외는 무귀속 — 해설자의 말. 그러므로 **이름을 불러야 할 소스는 한 구간에 claims 를 모아 앵커로 만든다.** 한두 claim 만 스치는 소스는 앵커가 될 수 없고 대본에서 이름 없이 지나간다 — 그런 소스가 많으면 소스를 줄인다(**사용 소스 수 ≤ 본문 구간 수 + 1**).
