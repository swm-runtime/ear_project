...
// full-v7.2: 금지 문구를 인용하지 않는다 — "이 문장은 쓰지 마"가 변형을 유도한다(실측). 문구 목록은 L0(goldRe)·루브릭 G1 만 갖는다
const GOLD_USAGE = `   **골드 사용법 (중요)**: 골드에서 배울 것은 구조·리듬·기법뿐이다. 골드의 **구체 비유·확인구·역질문 진입·피벗·도입 형태·마무리 마지막 문장은 재사용 금지** — 자리만 배우고 같은 기법을 새 재료·새 문장으로 구현하라.`;
...
${t.intro}
\`\`\`
     · {대주제 한 줄} = ${majorLine ? `"${majorLine}" (확정 문구 — 토씨 그대로 사용)` : `대분류 "${i.majorTopic ?? "-"}"를 소개하는 한 줄을 **직접 지어 쓴다** (확정 문구 미정 상태). 전문가 대상 표현 금지 — 누구나 듣는 한 줄. 한 문장, 담백하게 (예: "출근길에 채우는 배움 한 조각 전하러 왔습니다.")`}
...
export function buildDraftPrompt(i: DraftInput): string {
  ...
  return `당신은 오디오 콘텐츠 서비스 "이어(ear)"의 대본 작가다. 청취자는 이 주제를 처음 듣는 일반 청취자다 (전문가 대상 표현 금지). 15분 분량의 2인 대화 팟캐스트 에피소드 대본을 만든다.
...
403·접근 실패 시 우회 금지 — 해당 소스 제외하고 진행 (최소 3건 유지, 미달 시 중단·보고). 영어 소스이면 대본은 전량 한국어 재서술. 직접 인용은 따옴표로 읽지 않고 풀어 말한다(규칙 20).
...
2. **플래그** — 2.1의 판단 항목 23개(A1~A10·B2·C1·C2·C3·C5·C6·D1·D3·F1·F2·F4·F5·G1)만. A9(귀속 과밀·소스 순회)·A10(독후감 화법)은 v2.2 신설 — 대본이 주제가 아니라 소스를 소개하거나 읽은 감상을 말하는 것처럼 들리는가로 판단한다. **2.2의 이관 항목(B1·B3·B4·C4·D2·D4·D5·F3·F6)은 플래그하지 않는다** — 코드와 QA가 잰다. 강도는 위반/의심, 15건 이내, 자구 인용 필수, "판정(사람)"·"사유" 열은 비운다.
...
1. **점수 (100점)** — 3장의 하위 항목 12개를 **각각 독립적으로** 채점한다. 항목마다 구간 정의를 대본과 대조하고, 점수 옆에 반드시 대본 자구를 \`[E12] "…"\` 형식으로 인용한다 (인용 없는 점수는 무효). 합계는 계산 결과일 뿐 — 합계를 보고 조정하지 않는다. 몰입(3.10)은 다른 축의 합으로 역산하지 말고 통으로 판단한다.
   ...
   - 만점 구간은 **골드 2종보다 그 항목에서 명백히 나을 때만** 준다. 평범하면 평범하다고 쓴다.
   - 리포트의 "사람 점수"·"사람 사유" 열은 비워 둔다.
2. **플래그** — 2.1의 판단 항목 23개(A1~A10·B2·C1·C2·C3·C5·C6·D1·D3·F1·F2·F4·F5·G1)만. A9(귀속 과밀·소스 순회)·A10(독후감 화법)은 대본이 주제가 아니라 소스를 소개하거나 읽은 감상을 말하는 것처럼 들리는가로 판단한다. **2.2의 이관 항목(B1·B3·B4·C4·D2·D4·D5·F3·F6)은 플래그하지 않는다** — 코드와 QA가 잰다. 강도는 위반/의심, 15건 이내, 자구 인용 필수, "판정(사람)"·"사유" 열은 비운다.
   - G1: 골드 2종과 대조해 관용구·진입 문구·정리 문구의 재사용을 찾는다.
3. **⭐ 잘된 지점** 3~7건 (빈 판정 열 포함).
4. **집계** 행은 사람 판정 후 채우므로 플래그·⭐ 수만 적는다.
...
- 청취자는 특정하지 않는다 — 후보의 기준은 셋뿐: 주제가 하나로 서는가 · 듣는 사람이 흐름을 따라가며 이해할 수 있는가 · 소스가 그 주제를 풀어내는가. 타깃 정합은 적지 않는다.
...
표 한 개: \`| C## | 주장 (한국어, 한 문장) | 발췌 ID | 유형 | 구간 | 저명 | 의견 |\`. 유형은 수치·인용·고유명사·인과·정의·실천 중 하나. 구간 = 이 claim 을 쓰는 본문 구간 번호(같은 소스의 claims 는 전부 같은 구간, 축 소스만 둘) · 저명 = 주체가 소개 없이 통하는 인물·기관이면 ○ · 의견 = 특정인의 의견·해석·예측·논쟁적 주장이면 ○. 귀속 등급(귀속·없음)은 유형·의견에서 대본 단계가 도출한다(규칙 20, full-v7.1).
...
역할표
- 축 소스: S? (도입과 착지를 잇는 소스 하나 — 없으면 "없음". 이 소스만 두 구간에 걸칠 수 있다)
- 블록 순서의 근거: 구간 #1 → #n 이 축의 논리 단계인 이유 한 줄 (소스의 서술·발행 순서가 아니다. 마무리 정리 턴이 이 순서대로 블록당 한 줄을 인과로 이어 축으로 수렴한다)
...
구간 #1 — 소제목
  목적: 청취자에게 남길 것 한 줄
  질문: 이 구간이 답하는 축의 하위 질문 한 개 — 앞 구간 끝에서 청취자가 실제로 묻게 되는 것 (full-v8.1)
  재료: C01, C03 (S1) · C07 (S3)   ← **소스는 한 구간에만 배정한다**(축 소스만 예외). 한 구간에 소스가 여럿이어도, 하나여도 된다. 사례·일화 재료는 구간당 하나만(예시 과다 방지)
  진행자 질문: 기원 서사가 붙은 질문 한 개 (왜 궁금해졌는지 한 조각 + 질문)
  전환 장치: 번역 맞장구로 닫기 | 인용구 던지기 | 되물음 | 딴 얘기 끼어들기 | 역질문 중 하나
  비율: n%
구간 #2 … (본문 구간 4~6개. 도입은 구간에 넣지 않는다 — [도입] 구역이 따로 맡는다. 소스의 서술 순서를 그대로 구간 순서로 삼지 않는다 — 기사 순서가 구조가 되면 나열이 이해를 앞선다. **축의 하위 질문에 답하지 않는 소스는 쓰지 않는다** — 소스 3건 하한만 지키면 된다. 남은 소스를 소화하려고 구간을 만들지 않는다)
...
- **귀속은 축 소스의 소개 한 번뿐이다** (guidelines 규칙 20~22, full-v8 2026-09-18 — 출처 고지는 대본 밖이 담당한다):
  ...
  · **소개 뒤로는 해설자가 아는 것으로 말한다** — "연구팀은 ~라고 봐요", "저자들 말로는", "이 교수는 ~라고 말해요", "~라고 보고됐어요", "~로 추정돼요", "~에 따르면"을 문장마다 붙이지 않는다. 수치·결과·의견도 마찬가지다(v8: claims 의 \`귀속\` 열은 참고 정보이지 문장마다 따르는 지시가 아니다). 남의 판단을 해설자의 판단과 가를 자리에서만 블록당 한 번 "이 연구는 그렇게 봐요"로 가리킨다.
  · **한두 턴에 스치는 소스는 귀속 없이 내용만 말한다** — "한 조사에서", "어떤 연구에 따르면"도 붙이지 않는다.
  · **헤지는 claims 를 그대로** — 한 연구의 결과를 정설로 올리지 않는 방법은 출처가 아니라 양태("~일 수 있다", "경향이 있다")다.
  · **직접 인용은 풀어 말한다** — 따옴표로 남의 문장을 읽어 주지 않고 내용을 해설자의 문장으로 옮긴다. 화자 없는 인용 예고("이런 문장이 있어요")도 금지. 예외는 사례로 읽어 주는 실제 문장(예시문·광고 문구·설문 문항)뿐.
  · **이름은 선택이다** — 그 이름 자체가 정보일 때만(저명 주체, 사례 당사자). 부를 때는 소스당 하나, 소개 뒤 재언급은 지시어("이 사람", "연구팀"), 풀네임 재호명은 블록에서 최대 한 번, 형태는 첫 등장 그대로(성만 따로 부르지 않는다). **매체명·기관명은 sources.md 에 있는 것만** — 없으면 익명으로.
  · **각주는 말하지 않는다** — 발행 시기·수행 장소·학술지·소스 안의 인용 관계·서지 통계·저자의 사전 기대·연구 제안·반복 조사의 표본 수 열거·통계 절차. 연구가 무엇을 어떻게 했는지의 한 문장(대상·기간·방법)은 내용이다.
  · **블록의 마지막 해설 턴은 그 블록이 축에 보태는 한 문장으로 닫는다**(구성안 "구간 목적"). **착지 구간은 새 소스를 소개하지 않는다** — 앞 블록의 내용을 되짚어 축으로 올린다.
  · **마무리 정리 턴이 수렴의 자리** — 블록마다 한 줄씩을 "그래서·반대로·그런데"로 앞 줄에 걸어 잇고 마지막 줄이 축 문장. 열거("A였고, B였고")가 아니라 인과. claims 를 새로 쓰지 않고 이름을 부르지 않는다. 축을 회수한 문장에서 끝난다(사족 없음). 클로징은 청취자에게 시키지 않는다("~해 보세요" ✗) — 진행자가 자기에게 하는 말로 닫되 문장 틀은 매번 새로(규칙 22, full-v8.1: 예시 문구를 적으면 그대로 복제된다).
  · 자기 점검: 소스를 블록 밖에서 쓰지 않았는가 · 세 턴 이상 이끄는 소스의 블록 첫 턴에 소개가 있는가 · 소개 뒤에 전언체·주어형 귀속이 이어지지 않는가(귀속·전언 표현이 든 해설 턴 네 턴 연속이면 L0 가 잡는다 — 어미만 걷어내지 말고 해설자의 말로) · 같은 이름을 세 번 이상 부르지 않았는가 · 소스 목록에 없는 이름이 없는가 · 따옴표 인용이 없는가.
...
export function buildWritePromptParts(i: WriteInput): PromptParts {
  ...
  const host = explainer === "윤아" ? "이음" : "윤아";
  const fence = (s: string) => "````\n" + s.trim() + "\n````";
  const system = `당신은 오디오 콘텐츠 서비스 "이어(ear)"의 **대본 작가**다. 청취자는 이 주제를 처음 듣는 일반 청취자다 (전문가 대상 표현 금지 — 특정 연령·직군으로 좁히지 않는다). 2인 대화 팟캐스트 대본을 쓴다.
...
톤·리듬·티키타카는 guidelines 규칙(1~10·19)과 spec/04 3장 페르소나만 따른다. 참조할 문장이 없으므로 확인구·정리 진입·클로징 문장은 규칙과 템플릿 골격대로 새로 쓴다.`}
...
- **비유 (규칙 3·18)**: 진행 턴이 비유를 지어내지 않았는가. 비유가 어려운 대목에만, 에피소드에 셋 이내인가. 개념 사이의 관계를 다른 사물의 관계로 옮긴 비유(배터리/충전기 류)는 없는가. 비유로 들어갈 때 무엇을 옮기는지 말했는가("이 상황을 ~로 옮기면"). 다른 구간의 장면을 되부르는 콜백이 없는가.
- **진행 턴 (규칙 25)**: 앞 질문 뒤에 꼬리 질문("뭐가 멈춘다는 거예요?")이 붙지 않았는가. 이해했으면 확인형("~에 가깝다는 거죠?")으로 받았는가. 진행자가 예시·비유를 만들지 않았는가 (L0 가 물음표 셋 이상·비유 표지를 잡는다).
- **귀속·소스 (규칙 20~22, v8)**: 소스를 claims 의 블록 밖에서 쓰지 않았는가. 세 턴 이상 이끄는 소스의 블록은 첫 해설 턴이 소개 한 문장인가, 마지막 해설 턴이 축에 보태는 문장인가. 소개 뒤로는 해설자의 말인가 — "연구팀은/저자들은/~라고 보고됐어요"가 문장마다 붙지 않았는가, 스치는 소스에 귀속을 달지 않았는가, 따옴표 인용을 풀어 말했는가. 이름은 정보일 때만, 소스당 하나, 세 번 이상 반복하지 않았는가, 소스 목록에 있는 이름인가. 착지에 새 소스가 없는가. 마무리가 블록당 한 줄을 인과로 이어 축으로 수렴하는가(열거 아님, claims·이름 없음, 청취자에게 시키지 않음). 퍼센트/퍼센트포인트 표기가 한 대본 안에서 같은가(규칙 24).
- **턴 끝**: 모든 턴이 완결된 문장으로 끝나는가 — 말줄임표·미완 구로 끝난 턴은 오디오가 끊긴 인상을 준다.
- **용어 (규칙 24)**: 청취자(이 주제의 비전공자)가 모를 전문 용어·외국어 개념·통계 표현을 전부 terms 에 적는다 — 한 턴에 하나, 그 자리에서 한 문장으로 풀었거나 다음 진행 턴이 되물어 풀게 했는가. 풀지 못한 용어는 이름 없이 현상으로 바꿔 쓴다. 논문 결과 문장("평균적으로 더 큰 순유출이 났다", "공통 흐름이 반대로 움직였다")은 말로 옮긴다("돈이 더 많이 빠져나갔다", "한쪽이 오르면 다른 쪽이 내렸다"). (판정 6편 공통 저점: "어려운 말이 많은데 풀어가는 과정이 없다")
...
- **귀속 등급(귀속·없음)은 당신이 고르지 않는다 — 워커가 type·opinion 에서 도출한다.** v8(guidelines 규칙 20)에서 이 열은 참고 정보다 — 대본의 귀속은 축 소스(세 턴 이상 이끄는 소스)의 소개 한 문장뿐이고 문장 단위 귀속은 없다. 대본은 소스를 이름으로 구별하지 않으므로(블록 하나 = 소스 하나, 규칙 22) **이름을 위해 claims 를 모을 필요가 없다.** 스치는 소스는 소스를 줄인다(**사용 소스 수 ≤ 본문 구간 수 + 1**). notable 은 "이름을 불러도 정보가 되는 주체"라는 표시일 뿐 등급을 바꾸지 않는다.
- 소스 사이를 잇는 해석("A와 B는 같은 원리다")은 여기 적지 않는다 — 구성안의 "연결·비유" 목록으로.
- 소스 안의 개인 사례(특정 직원의 일화)는 논지에 꼭 필요할 때만 claims 에 넣고, 직장·연차·이니셜 같은 식별 정보는 text 에서 뺀다(규칙 20).
...
export const DESIGN_INLINE_SCHEMA = {
  ...
  properties: {
    ...
    claims: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "text", "excerpt_ids", "type", "section", "notable", "opinion"], properties: { id: { type: "string" }, text: { type: "string" }, excerpt_ids: strArr, type: { type: "string", enum: ["수치", "인용", "고유명사", "인과", "정의", "실천"] }, section: { type: "integer", description: "이 claim 을 쓰는 본문 구간 번호 — 같은 소스의 claims 는 전부 같은 구간(axis_source 만 도입·착지 두 구간)" }, notable: { type: "boolean", description: "주체가 청취자가 소개 없이 아는 저명 인물·기관인가 (프로이트·하버드·연준 수준. 일반 대학·연구소·매체·개별 연구자는 false)" }, opinion: { type: "boolean", description: "특정인의 의견·해석·예측, 또는 논쟁적·이해당사자 주장인가" } } } },
...
export const AXIS_CHECK_SCHEMA = {
  ...
  properties: {
    axis: { type: "string", description: "구성안의 축 한 문장 (그대로 옮김)" },
    sections: { type: "array", items: { type: "object", additionalProperties: false, required: ["n", "question", "answers_axis", "why"], properties: {
      n: { type: "integer" }, question: { type: "string", description: "그 구간의 \`질문:\` 줄 (없으면 목적 줄)" },
      answers_axis: { type: "boolean", description: "이 질문에 답하면 축의 질문에 한 걸음 가까워지는가. 인접 주제로 번진 것(같은 상위 개념이지만 축의 대상이 아닌 것)은 false" },
      why: { type: "string", description: "한 문장" } } } },
    verdict: { type: "string", enum: ["pass", "fail"], description: "false 가 하나라도 있으면 fail" },
    note: { type: "string", description: "fail 이면 어느 구간을 빼거나 축을 어떻게 다시 세워야 하는지 두 문장" },
  },
} as const;
export function buildAxisCheckPromptParts(i: AxisCheckInput): PromptParts {
  const system = `당신은 오디오 콘텐츠 서비스 "이어(ear)"의 구성안 심사자다. 대본을 쓰지 않는다. 구성안의 **축**(대립·역설·재정의 한 문장)과 **구간마다의 질문**을 대조해, 각 구간이 축의 질문에 답하는 단계인지 판정한다.
판정 기준:
- 축은 청취자가 15분 동안 붙드는 하나의 질문이다. 구간의 질문은 그 질문을 향해 한 걸음씩 가는 하위 질문이어야 한다.
