...
- **귀속은 축 소스의 소개 한 번뿐이다** (guidelines 규칙 20~22, full-v8 2026-09-18 — 출처 고지는 대본 밖이 담당한다):
  ...
  · 자기 점검: 소스를 블록 밖에서 쓰지 않았는가 · 세 턴 이상 이끄는 소스의 블록 첫 턴에 소개가 있는가 · 소개 뒤에 전언체·주어형 귀속이 이어지지 않는가(귀속 표현이 든 해설 턴 다섯 턴 연속이면 L0 가 잡는다 — 어미만 걷어내지 말고 해설자의 말로) · 같은 이름을 세 번 이상 부르지 않았는가 · 소스 목록에 없는 이름이 없는가 · 따옴표 인용이 없는가.
