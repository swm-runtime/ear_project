...
export default async function SettingsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="설정" breadcrumb={["파이프라인", "설정"]} desc="TTS 보이스·시그니처 템플릿·썸네일 띠 색·워커 기본값. 규칙 자산(가이드라인·골드·루브릭·QA 프롬프트)은 규칙 자산 화면에서 버전 관리·활성화한다 (spec/10 3.2)." />
      <SettingsForm tts={s.tts?.value ?? {}} worker={s.worker?.value ?? {}} templates={s.templates?.value ?? {}}
        thumbnail={s["thumbnail.colors"]?.value ?? {}} anchor={s["thumbnail.anchor"]?.value ?? null}
        meta={{ tts: s.tts, worker: s.worker, templates: s.templates, "thumbnail.colors": s["thumbnail.colors"] }} />
