  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-md border border-line bg-panel p-4 text-[13px] shadow-[0_1px_2px_rgba(38,49,61,0.04)]">
      ...
      <button className={btnCls()} disabled={pending || !mid}
        onClick={() => start(async () => { try { const id = await enqueueJob("cluster", { mid_topic: mid }); setMsg(`군집화 v2 요청됨 — 중분류 풀 (${id.slice(0, 8)})`); } catch (e: any) { setMsg(e.message); } })}>군집화 v2 (중분류 풀)</button>
      <button className={btnCls()} disabled={pending} title="군집화 v2 — 축(대립·역설·재정의)을 먼저 세우고 역할·다양성으로 후보를 만든다. 중분류가 속한 대분류 풀 전체에서 찾는다 (spec/03 2장 v2)"
        onClick={() => start(async () => { try { const id = await enqueueJob("cluster", { mid_topic: mid, cluster_version: "v2", major_topic: majorOf(mid) }); setMsg(`군집화 v2 요청됨 — 대분류 풀 (${id.slice(0, 8)})`); } catch (e: any) { setMsg(e.message); } })}>군집화 v2 (대분류 풀)</button>
