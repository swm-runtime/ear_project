...
export const cfg = {
  ...
  /** 비평 실행 형태 (2026-09-16 비용): single = 루브릭·규칙·골드 인라인(시스템 블록 캐시)·도구 없음·리포트는 JSON 으로 · agent = 파일 Read·Write 루프(기본). 측정 작업은 payload.mode 로 지정 */
  criticMode: (process.env.CRITIC_MODE === "single" ? "single" : "agent") as "single" | "agent",
