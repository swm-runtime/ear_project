...
export type JobType = "sweep" | "cluster" | "draft" | "qa" | "critic" | "tts" | "package" | "domain_check" | "thumbnail" | "critic_measure" | "enrich" | "script_align";
...
export async function claimJob(worker: string, canAi: boolean, canTts: boolean, canThumbnail: boolean, excludeTypes: JobType[] = []): Promise<Job | null> {
  const r = await pool.query("select * from public.claim_job($1, $2, $3, $4, $5)", [worker, canAi, canTts, canThumbnail, excludeTypes]);
