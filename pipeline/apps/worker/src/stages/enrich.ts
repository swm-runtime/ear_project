...
export async function runEnrich(job: Job, ex: Executor) {
  ...
  if (!/^[A-Za-z0-9-]{1,64}$/.test(contentId)) throw new Error("payload.content_id 필요");
  const episodeId = job.payload.episode_id ? String(job.payload.episode_id) : null;
  const title = String(job.payload.title ?? ""), description = String(job.payload.description ?? "");
  const topicNames = Array.isArray(job.payload.topic_names) ? (job.payload.topic_names as unknown[]).map(String) : [];
  ...
  await setJobProgress(job.id, { phase: `메타 부여 — 입력 수집 (${contentId.slice(0, 8)})`, detail: episodeId ? `대본 ${episodeId}` : "폴백(제목+설명)", toolCounts: {}, turns: 0, elapsedMs: 0 }).catch(() => {});
  let script: string | null = null;
  if (episodeId) {
    script = await getFile(`episodes/${episodeId}/script.md`);
    if (!script) log(`  enrich ${contentId.slice(0, 8)}: ${episodeId}/script.md 없음 — 폴백(제목+설명)으로`);
  }
  ...
  await setJobProgress(job.id, { phase: `메타 부여 — 판정 (${contentId.slice(0, 8)})`, detail: `${script ? `대본 ${script.length}자` : "폴백"} · ${cfg.enrichModel}`, toolCounts: {}, turns: 0, elapsedMs: 0 }).catch(() => {});
  const r = await ex.run<Record<string, unknown> & { evidence?: Record<string, string> }>({ prompt, schema: ENRICH_SCHEMA, tools: [], allowedTools: [], cwd: cfg.workRoot, timeoutMs: 10 * 60_000, model: cfg.enrichModel, effort: "medium",
    onProgress: (pr) => setJobProgress(job.id, { ...pr, phase: `메타 부여 — 판정 (${contentId.slice(0, 8)})` }).catch(() => {}) });
  ...
  await putFile(`datasets/enrichment/${contentId}.report.json`, JSON.stringify(report, null, 1));
  if (!n.file) {
    await insertRun({ phase: "enrich", result: `메타 부여 실패 ${contentId.slice(0, 8)} "${title.slice(0, 30)}" — ${n.errors.join("; ").slice(0, 300)} (enum 밖 값은 산출물을 내지 않는다, 명세 7장)`, prompt_version: `enrich-v1 (schema ${ENRICHMENT_SCHEMA_VERSION})`, artifacts: [s3Key(`datasets/enrichment/${contentId}.report.json`)], executed_by: executedBy, model: r.model, cost_usd: r.listCostUsd, tokens: (r.raw as { usage?: unknown } | undefined)?.usage, worker_rev: workerRev() });
    throw new Error(`메타 판정이 값 집합을 벗어남: ${n.errors.join("; ")}`);
  }
  const key = ENRICH_KEY(contentId);
  await putFile(key, JSON.stringify(n.file, null, 2) + "\n");
  const summary = `메타 부여 ${contentId.slice(0, 8)} "${title.slice(0, 30)}" — ${[n.file.difficulty, n.file.format, n.file.is_evergreen == null ? null : n.file.is_evergreen ? "evergreen" : "시의성", n.file.keywords ? `키워드 ${n.file.keywords.length}` : null, n.file.target_audiences ? `청자 ${n.file.target_audiences.length}세트` : null].filter(Boolean).join(" · ")}${n.file.source ? " · 폴백(제목+설명)" : ""}${n.warnings.length ? ` · ${n.warnings.join(" / ").slice(0, 200)}` : ""} · 반영 대기(콘솔 [반영])`;
  await insertRun({ phase: "enrich", result: summary, prompt_version: `enrich-v1 (schema ${ENRICHMENT_SCHEMA_VERSION})`, artifacts: [s3Key(key), s3Key(`datasets/enrichment/${contentId}.report.json`)], executed_by: executedBy, model: r.model, cost_usd: r.listCostUsd, tokens: (r.raw as { usage?: unknown } | undefined)?.usage, worker_rev: workerRev() });
  log(`  enrich: ${summary} ($${(r.listCostUsd ?? 0).toFixed(2)})`);
  return { content_id: contentId, key, file: n.file, warnings: n.warnings, fallback: !script, list_cost_usd: r.listCostUsd };
}
