...
/** 썸네일 스타일 앵커 참조 지시 (KAN-50) — 10번째 DB 자산. 앵커가 지정됐을 때만 본 프롬프트 뒤에 붙는다.
 *  별도 자산으로 둔 이유: 참조 강도는 실측하며 몇 번 고치게 되는데, 코드에 두면 그때마다 배포가 돈다 */
export const THUMBNAIL_ANCHOR_PROMPT_KEY = "skills/thumbnail/anchor.md";
...
export function versionOf(key: string, content: string): string {
  ...
  const m = head.match(/(full|qa|critic|thumb|anchor)-v\d+(?:\.\d+)*/);
  if (!m) throw new Error(`${key}: 헤더에서 버전 라벨을 찾지 못했다 (full-vN · qa-vN · critic-vN · thumb-vN · anchor-vN)`);
  return m[0];
}
