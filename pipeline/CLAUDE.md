...
- **규칙 자산 8개(guidelines·골드 3·QA 프롬프트·루브릭 v1/v2·TTS 음차 사전)의 진실은 DB `prompt_assets`다**(spec/10 3.2). 고칠 때는 웹 `/assets`에서 새 버전(draft) → 활성화. `docs/ai/skills/`의 사본은 `npm run assets:export`로 내려받는 스냅샷이므로 git에서 직접 고치지 않는다(고쳤다면 `assets:import --force`로 명시 게시). spec/03·04·05 본문은 git이 진실이며 워커가 체크아웃에서 읽는다.
