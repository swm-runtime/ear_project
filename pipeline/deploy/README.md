...
- **대본 임베딩**(2026-09-22, metadata-pipeline 4.3): env.prod 에 `AI_SERVER_TOKEN`(= env.ai-server 의 `INTERNAL_AUTH_TOKEN`)을 넣고 `docker compose up -d worker-io web` 으로 재기동한다. env.ai-server 는 `EMBEDDING_PROVIDER=openai` 여야 한다(stub 이면 벡터를 저장하지 않는다). 비우면 임베딩 없이 발행되고 실행 기록에 사유가 남는다.
